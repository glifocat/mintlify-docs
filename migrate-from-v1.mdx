...
{/* verified-against: migrate-v2.sh, migrate-v2-reset.sh, setup/migrate-v2/*.ts, .claude/skills/migrate-from-v1/SKILL.md, .claude/skills/migrate-from-openclaw/SKILL.md, .claude/skills/migrate-nanoclaw/SKILL.md, .claude/skills/migrate-memory/SKILL.md @ 4e83a0a0 (main) */}
...
| Group folders | Copied into `groups/`. Each v1 `CLAUDE.md` is staged as `CLAUDE.local.md` — a temporary legacy file the post-migration skill later distills into v2's real surfaces (`instructions.prepend.md` for standing instructions, the `memory/` tree for durable facts); v2 composes `CLAUDE.md` fresh at container spawn. v1 `container_config` becomes `container.json` (or a `.v1-container-config.json` sidecar if it can't be parsed). |
...
The migration is two actors in sequence: the script does everything deterministic, then hands off to Claude for the parts that need judgment.
...
flowchart LR
    S["migrate-v2.sh<br/>phases 0–3"] --> W["service switchover"] --> H["handoff.json"] --> K["/migrate-from-v1 skill"] --> V["verify + restart"]
*/}
<img className="block dark:hidden" src="/images/diagrams/migrate-from-v1-light.svg" alt="Two-lane migration: the deterministic idempotent migrate-v2.sh phases and service switchover, handing off through handoff.json to the Claude judgment lane with the migrate-from-v1 skill and final verification; the v1 checkout is read-only" />
<img className="hidden dark:block" src="/images/diagrams/migrate-from-v1-dark.svg" alt="Two-lane migration: the deterministic idempotent migrate-v2.sh phases and service switchover, handing off through handoff.json to the Claude judgment lane with the migrate-from-v1 skill and final verification; the v1 checkout is read-only" />
...
<Steps>
  ...
  <Step title="Phase 3: infrastructure">
    Installs Docker if missing, sets up the OneCLI credential vault, registers your Anthropic credential (skipped if `.env` already has `ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN`), copies v1 container skills v2 doesn't have, and acquires the agent container image — it builds one unless `NANOCLAW_HARDENED_IMAGE` is already set in `.env`, in which case it fetches the pinned image instead (see [Get started with the Echo hardened runtime](/guides/echo-hardened-runtime)).
  </Step>
...
3. **Migrate legacy memory.** The migration staged your entire v1 `CLAUDE.md` per group as `CLAUDE.local.md`. The skill runs `/migrate-memory` for each imported group: it quiesces the group, moves the staged file into the shared `memory/` tree, then distills standing identity into `instructions.prepend.md` and durable facts into memory files before the group runs again.
...
| `IDENTITY.md` / `SOUL.md` | Standing behavior woven into the group's `instructions.prepend.md`; backstory and reference material become linked concepts under `memory/` |
| `USER.md`, `MEMORY.md`, daily memory files | Focused concepts in the group's `memory/` tree (always-relevant facts go to Core Memory); daily files are copied into a memory folder or consolidated, your choice |
...
| Cron jobs | v2 scheduled tasks, created with the [`ncl tasks` CLI](/reference/ncl-cli#tasks) and run in isolated per-task sessions |
