...
4. Group folder and logs directory created
5. `CLAUDE.md` template copied from `groups/main/` (for main) or `groups/global/` (for non-main) if no `CLAUDE.md` exists yet — the assistant name is substituted automatically
6. A corresponding OneCLI agent is created for per-group credential scoping (best-effort, non-blocking)
7. Group added to SQLite database
...
- **Per-group override**: Each group can define its own trigger via the `trigger` field in `RegisteredGroup`
- **Configured via**: `ASSISTANT_NAME` environment variable (for the default)
- **Example**: `@Andy` if `ASSISTANT_NAME=Andy`, or `@Claw` if the group sets a custom trigger
- **Can be disabled**: Set `requiresTrigger: false` in group config

**Per-group triggers:**

Groups can use a custom trigger word that differs from the global default. When a group has a custom `trigger` set, only that trigger activates the agent — the default trigger is ignored for that group. This allows different groups to use different names for the same assistant.
...
<Note>
Triggers check both message content and sender authorization. A message must match the trigger pattern **and** the sender must be allowed by the [sender allowlist](/concepts/security#5-sender-allowlist) (specifically, `isTriggerAllowed` checks against the allowlist configuration). Messages from the bot itself (`is_from_me`) always pass the sender check.
</Note>
...
3. `CLAUDE.md` template auto-copied from `groups/main/` or `groups/global/` (assistant name substituted)
4. OneCLI agent created for per-group credential scoping (non-main only)
5. Session directory created under `data/sessions/{folder}/`
6. IPC directory created under `data/ipc/{folder}/`
7. Skills synced to `.claude/skills/`
8. Agent runner source copied to `agent-runner-src/`
9. Group added to database
