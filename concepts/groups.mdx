...
interface RegisteredGroup {
  ...
  trigger: string;           // Per-group trigger (e.g., "@Claw"); overrides default @{ASSISTANT_NAME}
...
- **Per-group override**: Each group can set a custom `trigger` field (e.g., `@Claw`) that takes precedence over the default
- **Configured via**: `ASSISTANT_NAME` environment variable (for the default), or the group's `trigger` field at registration
- **Example**: `@Andy` if `ASSISTANT_NAME=Andy`, or `@Claw` if the group's trigger is set to `@Claw`
- **Can be disabled**: Set `requiresTrigger: false` in group config

When a custom per-group trigger is set, only that trigger activates the agent for that group — the default `@{ASSISTANT_NAME}` trigger will not match.
