...
2. `spawnContainer(session)` resolves the provider (`sessions.agent_provider` → `agent_groups.agent_provider` → `container.json` `provider` → `'claude'`), reads agent group config, and builds mounts
