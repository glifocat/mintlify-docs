...
5. **Graceful shutdown**: `docker stop -t 1` (SIGTERM with 1-second grace). If the graceful stop fails, the process is force-killed via `SIGKILL`
...
- **Enforcement**: `docker stop -t 1` (SIGTERM with 1-second grace). Falls back to `SIGKILL` if the graceful stop fails
