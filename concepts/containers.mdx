...
<Note>
The agent-runner is recompiled on every container start from `/app/src` (mounted from `data/sessions/{group}/agent-runner-src/`). This allows agents to modify their own tools and behavior by editing their agent-runner source. The agent-runner source is copied from `container/agent-runner/src/` when the upstream `index.ts` is newer than the group's cached copy — so upstream changes propagate automatically on the next container spawn, but per-group customizations are preserved until the upstream source is updated.
</Note>
...
3. **Per-group customization**: Each group gets a copy, but changes are overwritten on next container spawn (unlike agent-runner source, which only updates when the upstream source is newer)
