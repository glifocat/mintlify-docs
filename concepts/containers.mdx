...
mounts = [
  ...
  {
    hostPath: '/path/to/nanoclaw/store',
...
- Built-in: `nanoclaw` MCP server (8 tools: `send_message`, `schedule_task`, `list_tasks`, `pause_task`, `resume_task`, `cancel_task`, `update_task`, `register_group`). The `send_message` tool accepts an optional `sender` parameter for named bot identities on supported channels (e.g., Telegram). The `register_group` tool accepts an optional `requiresTrigger` boolean (defaults to `false`) that controls whether messages must contain the trigger word to activate the agent.
