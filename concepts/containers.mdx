...
- Image tag: `nanoclaw-agent-v2-<slug>:{agentGroupId}`
- Built on top of the per-checkout base image `nanoclaw-agent-v2-<slug>:latest`
- Cached — only rebuilt when package lists change

The `<slug>` is `sha1(projectRoot)[:8]`, so two NanoClaw installs on the same host never share an image tag. Override with `CONTAINER_IMAGE_BASE` if you need to pin to a fixed name.
