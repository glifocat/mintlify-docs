...
<Note>
The `.env` file is explicitly shadowed by mounting `/dev/null` over it. This prevents agents from reading API keys or other secrets from the project root, even though the project is mounted read-only. Credentials are injected by the credential proxy instead.
</Note>
...
- **Apple Container** (macOS): Available via the `/convert-to-apple-container` skill

On Linux, the container runner automatically adds `--add-host=host.docker.internal:host-gateway` so containers can reach the host's credential proxy.

## Skills and MCP servers

Skills synced to each container:

1. **Copy from project**: Skills from `container/skills/` are synced to `data/sessions/{group}/.claude/skills/` for each container session
...
- **Credential exposure**: While `.env` is shadowed via `/dev/null`, the proxy URL is accessible from within the container's network
