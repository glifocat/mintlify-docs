...
The main group gets the project root mounted read-only, but the `.env` file is shadowed with `/dev/null` to prevent the agent from reading secrets:
  ...
  "allowedRoots": [
    {
      ...
      "description": "Website deployment tasks"
    }
  ],
  "blockedPatterns": [".ssh", ".gnupg", ".env"],
...
2. Check against blocked patterns (`.ssh`, `.env`, etc.) — custom patterns are merged with built-in defaults
3. Verify path falls under an `allowedRoots` entry
...
1. **Input passed**: Container input (prompt, group info) via stdin JSON
2. **Credentials routed**: API traffic goes through the host's credential proxy — containers get a placeholder token and proxy URL, never real credentials
3. **Stdout streaming**: Parsed for output markers in real-time
4. **Stderr logging**: Logged at debug level (SDK writes lots of debug info)
5. **Timeout tracking**: Hard timeout with activity-based reset
6. **Graceful shutdown**: `docker stop` (SIGTERM) before `SIGKILL`
...
1. **Copy from project**: Operational skills from `container/skills/` are synced to `data/sessions/{group}/.claude/skills/` for each container session
