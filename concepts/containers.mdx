...
mounts = [
  ...
  {
    // .env shadowed with /dev/null so agents cannot read secrets
    hostPath: '/dev/null',
...
<Note>
The `.env` file is shadowed with `/dev/null` in the project root mount. This prevents the agent from reading secrets directly — credentials are injected by the [credential proxy](/advanced/security-model#credential-handling) instead.
</Note>
...
{
  "allowedRoots": [
    { "path": "~/projects", "allowReadWrite": true, "description": "Project files" },
    { "path": "~/Documents/work", "allowReadWrite": false, "description": "Reference docs" }
  ],
...
2. Check against blocked patterns (`.ssh`, `.env`, `.gpg`, `.pypirc`, etc.)
3. Verify real path falls under an `allowedRoots` entry
4. Enforce read-only unless root has `allowReadWrite: true` and group is main (or `nonMainReadOnly` is false)
