...
{/* verified-against: src/db/schema.ts, src/db/migrations/{010-engage-modes,011-pending-sender-approvals,012-channel-registration,016-messaging-group-instance,019-wiring-threads}.ts, src/types.ts, src/router.ts, src/channels/channel-defaults.ts, src/session-manager.ts, src/modules/permissions/{index.ts,access.ts}, src/command-gate.ts @ 639577c3 */}
{/* verified-against (scoped correction) — adapter sender/session defaults and thread-policy validation: src/router.ts, src/channels/{channel-defaults,channel-registry}.ts, src/cli/resources/wirings.ts @ b76fcb3d */}
...
erDiagram
    ...
    WIRINGS {
        ...
        int threads "NULL inherit | 1 on | 0 off"
    }
    SESSIONS {
        string thread_id "nullable"
    }
*/}
<img className="block dark:hidden" src="/images/diagrams/entity-model-light.svg" alt="Five entities: agent groups and messaging groups joined by wirings, which hold the five behavior axes; sessions owned by agent groups and scoped by messaging groups; users with roles and membership as the access chain" />
<img className="hidden dark:block" src="/images/diagrams/entity-model-dark.svg" alt="Five entities: agent groups and messaging groups joined by wirings, which hold the five behavior axes; sessions owned by agent groups and scoped by messaging groups; users with roles and membership as the access chain" />

## Agent groups — the unit of isolation

An agent group is a workspace: a folder under `groups/<name>/` with its own working files, per-group standing instructions (`instructions.prepend.md`) and [durable memory](/concepts/agent-memory) (the `memory/` tree), a composed `CLAUDE.md`, and a container configuration (the `container_configs` table — packages, mounts, model and skills config). Every session an agent group owns runs in a container that sees only that workspace. All agent groups are equal — privilege lives on users, not on groups.

## Messaging groups — a platform conversation

A messaging group is one chat or channel on one platform, identified by `(channel_type, platform_id, instance)` — a Discord channel, a WhatsApp group, a DM. The `instance` names the adapter that owns the chat, so running several adapters of one platform (say three Slack apps in one workspace) keeps their chats distinct; it defaults to the channel type, so single-instance installs effectively key on `(channel_type, platform_id)` as before. It carries an `unknown_sender_policy` (`strict` / `request_approval` / `decline_notify` / `public`) and can be denied (`denied_at`), after which messages from it drop silently. Channel membership hooks can also mark it detached (`detached_at`) when the bot leaves; the wiring and history survive, but delivery stops until the bot rejoins.

You don't pre-register these. When a mention or DM arrives on a chat NanoClaw has never seen, the router auto-creates the row using the receiving adapter's declared DM or group sender policy (`request_approval` for undeclared adapters) — and if no agent is wired to it, escalates to an approver (group admins first, then global admins, then owners) with an approve/deny card before any agent responds. Plain chatter in unwired channels is ignored entirely.

## Wirings — where the behavior lives

A wiring (`messaging_group_agents` row) connects one messaging group to one agent group. It's many-to-many: one chat can feed several agents, one agent can serve several chats. The wiring — not the agent, not the chat — holds the five orthogonal behavior axes:
...
### `threads` — honoring platform threads

Unless explicitly overridden, new wirings inherit the adapter's declared `sessionMode`; a declared `per-thread` default also stamps `threads = 1`. An undeclared adapter falls back to `shared` with `threads = NULL`.

The `threads` field is a per-wiring override on whether inbound thread ids are honored for this wiring. `NULL` inherits the channel adapter's declared default for the context (DM versus group); `1` forces thread honoring on, `0` forces it off. It's hard-ANDed with the adapter's raw capability at router fanout: it can disable threads on a thread-capable platform, but never invent them on one that has none. Setting `threads = 0` also lifts the per-thread forcing described below.
...
1. **Thread-capable adapters force `per-thread` in group chats.** On Discord, Slack, or Teams channels, every thread gets its own session even if the wiring says `shared` — `agent-shared` overrides this (a cross-channel directive the adapter knows nothing about), and so does a per-wiring `threads = 0`. DMs are exempt: whether a DM's sub-threads collapse into one conversation or stay separate follows the channel's declared DM thread default and any per-wiring `threads` override, not `session_mode`.
...
Creating a wiring, or updating its `session_mode` or `threads`, rejects `per-thread` with explicit `threads = 0` or an inherited threads-off policy on a declared adapter. Undeclared adapters retain their legacy leniency when `threads` is unset. On a thread-capable channel whose context defaults to threads off, pair `--session-mode per-thread` with `--threads true`; that flag cannot invent thread support on a thread-less adapter. Older rows may still contain combinations that collapse at routing time, and unrelated updates to those rows remain allowed.

How the router resolves an existing wiring:
...
The DM column assumes the channel's default DM thread behavior; a per-wiring `threads` override changes it, and `threads = 0` also disables the forced `per-thread` in the group-chat column (see the [`threads` axis](#threads-—-honoring-platform-threads) above).
...
Each messaging group's `unknown_sender_policy` decides what happens when someone outside that chain writes in: `strict` drops silently, `request_approval` drops and DMs an approval card to an admin (approve makes them a member and replays the message), `decline_notify` politely declines an unknown DM sender and sends an owner or admin a one-line FYI, and `public` lets anyone through. On group chats, `decline_notify` behaves as `strict`. Full details, including auditing dropped messages, are in [hardening](/operate/hardening#restrict-who-can-talk-to-your-agents).
