...
Containers never see real credentials. The OneCLI gateway (`@onecli-sh/sdk`) runs on the host and intercepts HTTPS traffic to inject API keys or OAuth tokens into container API requests:

- The gateway configures container arguments at spawn time via `onecli.applyContainerConfig(args)`
...
<Warning>
The OneCLI gateway prevents credential exposure to containers. However, containers can still make authenticated API requests through the gateway (though they cannot extract real credentials).
</Warning>
