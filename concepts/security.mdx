...
<Warning>
**Breaking change (v1.2.35):** OneCLI Agent Vault is now the sole credential management system. The built-in credential proxy has been removed from the codebase. If you have credentials in `.env`, run `/init-onecli` to migrate them to the vault.
</Warning>
