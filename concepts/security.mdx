...
mounts = [
  ...
  { host: '/path/to/nanoclaw/store', container: '/workspace/project/store', readonly: false },
...
<Note>
The `store/` directory is mounted separately as read-write, overlaying the read-only project root. This gives the main agent direct access to the SQLite database for queries and writes.
</Note>
