...
- Containers receive a placeholder token and the proxy URL (`ANTHROPIC_BASE_URL`)
- Real credentials are injected by the credential proxy on the host, never exposed to containers
...
The proxy reads credentials from `.env` at startup and supports two authentication modes: API key (injects `x-api-key` on every request) and OAuth (replaces placeholder Bearer token with the real OAuth token on exchange requests).
