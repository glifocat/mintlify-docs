...
{/* verified-against: src/container-runner.ts, src/egress-lockdown.ts, src/inbox-safety.ts, src/session-manager.ts, src/modules/mount-security/index.ts, src/command-gate.ts, src/modules/permissions/{index.ts,access.ts,channel-approval.ts,sender-approval.ts}, src/modules/approvals/{index.ts,primitive.ts,onecli-approvals.ts,reason-capture.ts,finalize.ts,response-handler.ts}, src/modules/agent-to-agent/{message-gate.ts,agent-route.ts,db/agent-message-policies.ts}, src/modules/self-mod/index.ts, container/Dockerfile, pnpm-workspace.yaml, docs/SECURITY.md @ 639577c3 */}
{/* verified-against (scoped correction) — group-scoped history, plugin mounts, sender defaults and build-script allowlist: src/cli/resources/sessions.ts, src/modules/cross-session-context/history.ts, src/container-runner.ts, src/channels/{channel-defaults,channel-registry}.ts, src/router.ts, pnpm-workspace.yaml @ b76fcb3d */}
...
flowchart TD
    ...
    CG --> C["Container — one agent group's blast radius"]
...
*/}
<img className="block dark:hidden" src="/images/diagrams/security-light.svg" alt="Containment view of the security model: an assumed-injected agent inside the container boundary, sender policies and the command gate on the inbound side, and bounded filesystem, network, and privileged-action paths crossing out" />
<img className="hidden dark:block" src="/images/diagrams/security-dark.svg" alt="Containment view of the security model: an assumed-injected agent inside the container boundary, sender policies and the command gate on the inbound side, and bounded filesystem, network, and privileged-action paths crossing out" />

## Container isolation — the blast radius is one agent group

A compromised agent is a compromised *container*, and every active session gets its own. The container runs as the unprivileged `node` user (or your host uid/gid), and `--rm` removes the container after exit; mounted group and session state persists. Every spawn also carries the same hardening flags, unconditionally and with no per-group override: `--cap-drop=ALL` and `--security-opt no-new-privileges` (depth against a root-in-container path — under the normal user mapping the capability sets are already empty), `--init` so signals actually reach the runner instead of being discarded at PID 1, plus a configurable `--pids-limit` fork-bomb backstop that defaults to 2048. The image itself strips the setuid-root bit from every binary Debian ships (`mount`, `su`, `passwd`, and friends), none of which an unprivileged agent needs. Its filesystem view is exactly the [mount table](/concepts/container-lifecycle#spawn--the-world-rebuilt-every-time): the session folder, the group workspace, and read-only shared code. No host home directory, no `.ssh`, no Docker socket. Its own `container.json` and composed project document are re-mounted read-only on top of the writable workspace, so the agent can read its configuration but not rewrite it to grant itself more. Stamped plugin code under `plugins/` is also mounted read-only; runtime writes belong in the separate `plugin-data/` tree, which remains writable through the group mount.

One boundary the container does *not* draw: every session of an agent group mounts the same group workspace read-write, so what a compromised session reaches is the **group's** files and `memory/` tree, not just its own conversation's. Session databases and provider continuations stay separate, but an agent can retrieve other sessions' transcripts within its group through `ncl sessions history`. That command rejects cross-group reads. The honest unit of blast radius is therefore the agent group, which is why different trust domains belong in [separate agent groups](/concepts/isolation-levels).
...
Setup, per-group secret grants, and the credential admission rules are in [Credentials](/operate/credentials).
...
- **Sender policies** — each known chat carries an `unknown_sender_policy`: `strict` silently drops strangers, `request_approval` asks an admin before admitting them, `decline_notify` politely declines unknown DM senders and sends an owner or admin a one-line FYI, and `public` lets anyone in. On group chats, `decline_notify` falls back to a silent drop. Auto-created chats use the receiving adapter's declared DM or group policy; undeclared adapters fall back to `request_approval`.
...
The host process itself is in the trusted zone, so its dependencies matter: pnpm is configured with a three-day `minimumReleaseAge` (newly published package versions wait before they can be installed) and an `onlyBuiltDependencies` allowlist limiting install scripts to `esbuild`, `protobufjs`, and `sharp`.
...
- **Poison its own memory** — instructions written to its [`memory/` tree](/concepts/agent-memory) or `instructions.prepend.md` today shape every future session of that group.
