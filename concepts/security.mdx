...
tag: "UPDATED"
keywords: ["security", "container isolation", "trust model", "credential proxy"]
---
...
mounts = [
  ...
  { host: '/dev/null', container: '/workspace/project/.env', readonly: true },  // Shadow secrets
...
The `.env` file is shadowed by mounting `/dev/null` over it, preventing agents from reading API keys even though the project root is mounted read-only.
...
- **API key mode**: Proxy injects `x-api-key` header on every request
- **OAuth mode**: Container CLI exchanges its placeholder token for a temporary API key via the Anthropic OAuth endpoint; proxy injects the real OAuth token on that exchange request
