...
{
  "allowedRoots": [
    { "path": "~/projects/website", "allowReadWrite": true, "description": "Website deployment tasks" }
  ],
...
The proxy reads credentials from `.env` once at startup and determines the auth mode (API key or OAuth). In API key mode, it injects the `x-api-key` header on every request. In OAuth mode, it replaces the `Authorization: Bearer` header on exchange requests while letting post-exchange requests pass through.
