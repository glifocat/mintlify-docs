...
Containers never see real credentials. The OneCLI gateway (`@onecli-sh/sdk`) runs on the host and intercepts HTTPS traffic to inject API keys or OAuth tokens transparently:

- The gateway URL defaults to `http://localhost:10254` (configurable via `ONECLI_URL`)
- `onecli.applyContainerConfig()` injects the necessary environment variables into container arguments
- Each non-main group gets its own OneCLI agent identifier for per-group credential scoping
...
- Real API keys or OAuth tokens - injected by the OneCLI gateway, never in containers
- Any credentials matching blocked patterns

<Warning>
The OneCLI gateway prevents credential exposure to containers. The container cannot extract real credentials, but it can make authenticated API requests through the gateway. If the gateway is unreachable at container start, the container launches without credentials.
</Warning>
...
│  • Container lifecycle                                            │
│  • OneCLI gateway (credential injection)                            │
└────────────────────────────────┬─────────────────────────────────┘
                                 │
