...
<Tabs>
  ...
  <Tab title="Credential Proxy (legacy / Apple Container)">
    ...
    Containers never see real credentials. A credential proxy runs on the host and injects authentication on every outbound API request. For Docker users, apply the `/use-native-credential-proxy` skill to restore the built-in credential proxy. Apple Container users get this automatically — the `apple-container` branch already includes the credential proxy code, so `/use-native-credential-proxy` must not be applied on top.
