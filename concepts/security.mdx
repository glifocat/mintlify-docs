...
<Tabs>
  <Tab title="OneCLI Gateway (v1.2.22+)">
    NanoClaw uses the [OneCLI](https://onecli.sh) gateway for centralized secret management. Container-facing API keys (Anthropic, OpenAI, Parallel, etc.) are managed by the gateway, which intercepts outbound API traffic from containers and injects credentials at request time.

    - Secrets are registered once via `onecli secrets create` (CLI or dashboard)
    - Channel tokens (Telegram, Slack, Discord) remain in `.env` — they are used by the host process, not by containers
