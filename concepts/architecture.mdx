...
- **messages**: All messages with timestamps and bot message flags
- **chats**: Chat metadata (name, last activity, channel, is_group)
- **sessions**: Claude session IDs per group folder
- **registered_groups**: Active groups configuration (including `is_main` flag)
- **router_state**: Message cursors and last processed timestamps
- **scheduled_tasks**: Scheduled task definitions with context mode
...
2. **Database initialization**: Create tables if needed, run migrations
3. **State loading**: Restore message cursors, sessions, registered groups
4. **Remote control restore**: Re-establish any active remote control sessions
5. **Credential proxy**: Start HTTP proxy for credential injection
6. **Channel connection**: Connect to messaging channels, authenticate if needed
7. **Subsystem startup**: 
   - Task scheduler loop (60s interval)
   - IPC watcher (1s poll interval)
   - Message loop (2s poll interval)
8. **Recovery**: Check for unprocessed messages from previous crash
9. **Ready**: System begins processing messages and tasks
