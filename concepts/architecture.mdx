...
3. Pass prompt and metadata via stdin JSON (credentials handled by the OneCLI gateway, never passed here)
...
- Auto-updated from upstream source when the upstream has a newer modification time
...
4. **OneCLI agent sync**: Ensure each registered non-main group has a corresponding OneCLI agent for per-group credential scoping (best-effort, non-blocking — recovers from missed creates if OneCLI was down at registration time)
