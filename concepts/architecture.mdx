...
- **session_state** — persistent key/value store. Primary use: per-provider continuation IDs (`continuation:<provider>`) so each provider resumes its own conversation independently across container restarts and provider switches
