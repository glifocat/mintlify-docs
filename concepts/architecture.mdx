...
- Checks for trigger pattern (per-group trigger, or default `@{ASSISTANT_NAME}`)
- Maintains cursor state to track processed messages
- Routes messages to the appropriate group queue

<Info>
The main group (typically your self-chat) doesn't require a trigger - all messages are processed automatically. Non-main groups can define a custom trigger that overrides the default.
</Info>
...
- **registered_groups**: Active groups with folder, per-group trigger, container config, is_main flag
