...
3. Pass prompt and metadata via stdin JSON (credentials handled by OneCLI gateway, never passed here)
...
- **messages**: All messages with timestamps (queries capped at 200 per invocation)
...
4. **OneCLI agent sync**: Ensure OneCLI agents exist for all registered groups (recovers from missed creates if the gateway was down at registration time)
