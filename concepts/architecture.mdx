...
NanoClaw uses a factory registry pattern for messaging channels. Telegram ships on `main` as a core channel; other channels (WhatsApp, Discord, Slack, Gmail) are added via skills. Each channel self-registers at startup. Channels with missing credentials emit a warning and are skipped — no configuration file is needed to enable or disable channels.
