...
{/* verified-against (scoped correction) — mailbox and central-store seams, instance lookup, reconciliation, claims and heartbeat fallback: src/{index,router,host-sweep,reconcile-session,container-runner}.ts, src/mailbox/{compose,sqlite/session-db}.ts, src/db/compose.ts @ b76fcb3d */}

NanoClaw is one Node.js host process plus one Docker container per active session. The host owns the channels, the routing, and the container lifecycle; the agent runs inside the container and never touches the messaging platforms — that traffic is all host. (The agent's own network egress is a separate matter: open by default, [lockable](/operate/hardening#lock-down-network-egress).) Between them sits the core idea: **every message is a row in SQLite**. The shipped mailbox uses a SQLite pair: the host writes rows the container reads, and the container writes rows the host reads. Mailbox storage, the central database, and the session runtime each sit behind a separate implementation seam. Host-side message writes and runtime exit events also enqueue reconciliation; periodic polling and resync recover when those hints are lost.
...
| Host reconciliation | On mail writes and runtime exit hints; resync 60s after the previous tick drains | A keyed workqueue; resync covers all active sessions |
...
The shipped storage implementations use SQLite, split across two layers — the [database schema reference](/reference/db-schema) documents every table.
