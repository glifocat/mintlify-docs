...
2. **Messaging group lookup** — finds or auto-creates messaging groups on mentions/DMs, keyed by `(channel_type, platform_id)`. Rows store the platform ID in the exact shape the adapter emits — Chat SDK adapters are prefixed (`discord:…`, `telegram:…`), native adapters (WhatsApp, iMessage, Signal) are not
