...
{/* verified-against: src/session-manager.ts, src/router.ts, src/channels/channel-defaults.ts, src/container-runner.ts, .claude/skills/manage-channels/SKILL.md, src/cli/resources/wirings.ts @ 639577c3 */}
{/* verified-against (scoped correction) — same-group history access, composed base instructions, wiring validation: src/cli/resources/{sessions,wirings}.ts, src/modules/cross-session-context/history.ts, src/container-runner.ts, src/channels/channel-defaults.ts @ b76fcb3d */}
...
*/}
<img className="block dark:hidden" src="/images/diagrams/isolation-levels-light.svg" alt="Decision tree over three questions landing on four isolation levels, ordered most to least isolated: separate agent groups, per-thread, shared, agent-shared; the three session modes share the same agent workspace files" />
<img className="hidden dark:block" src="/images/diagrams/isolation-levels-dark.svg" alt="Decision tree over three questions landing on four isolation levels, ordered most to least isolated: separate agent groups, per-thread, shared, agent-shared; the three session modes share the same agent workspace files" />
