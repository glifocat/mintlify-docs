...
Intervals are anchored to the **scheduled time** (not the completion time) to prevent cumulative drift. If any intervals were missed while the task was running, they are skipped so the next run always lands in the future.

```typescript
// Anchor to the scheduled time, skip past missed intervals
...
- Task remains in database with execution history
- Will not run again unless status and `next_run` are manually updated
