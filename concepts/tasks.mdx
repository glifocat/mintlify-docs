...
Intervals are **anchored to the scheduled time**, not the current time. This prevents cumulative drift. If an interval is missed, the scheduler skips forward to the next future occurrence:
...
All task executions are logged to the `task_run_logs` table:

```typescript
interface TaskRunLog {
  task_id: string;      // References scheduled_tasks.id
