...
- Optionally gated by a bash script that decides whether to wake the agent
...
sequenceDiagram
    ...
    CR->>CR: Spawn container
    alt Task has script
        CR->>SC: Run bash script (30s timeout)
        SC-->>CR: { wakeAgent, data }
        alt wakeAgent = false
            CR->>DB: logTaskRun(skipped)
            CR->>DB: updateTaskAfterRun(next_run)
        end
    end
    CR->>AG: Start agent with prompt (+script data)
...
7. **Script phase** (if task has a `script`): bash script runs with a 30-second timeout. If the script returns `wakeAgent: false` or fails, the container exits early without starting the agent.
8. Agent executes task prompt (enriched with script data if the script returned `wakeAgent: true`)
9. Results automatically forwarded to the group chat when the agent produces output
10. Run logged to `task_run_logs` table with duration and result
11. `next_run` recalculated based on schedule type
12. Container closes after 10-second grace period
...
await claudeCode.tools.schedule_task({
  ...
  script: "curl -so /dev/null -w '%{http_code}' https://example.com | node -e \"const c=require('fs').readFileSync('/dev/stdin','utf8').trim();console.log(JSON.stringify({wakeAgent:c!=='200',data:{statusCode:c}}))\""
});
```

The optional `script` parameter runs a bash script before waking the agent. See [task scripts](/features/scheduled-tasks#task-scripts) for details.
...
Modify an existing task's prompt, schedule, or script:
...
await claudeCode.tools.update_task({
  ...
  script: "echo '{\"wakeAgent\": true}'" // Set to "" to remove script
});
...
4. **Use scripts for frequent tasks**: If a task runs more than a few times per day, add a script to gate agent wake-ups and reduce API costs
...
7. **Monitor task_run_logs**: Failed tasks may indicate issues
