...
- Resolved from `process.env.TZ`, `.env` file `TZ`, or system timezone — each validated as a real IANA identifier before accepting, with `UTC` as final fallback
...
interface ScheduledTask {
  ...
  script?: string | null; // Optional script to run instead of prompt
  // ... other fields
}
```
