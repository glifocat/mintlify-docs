...
{/* verified-against: src/channels/chat-sdk-bridge.ts, src/router.ts, src/channels/channel-defaults.ts, setup/channels/run-channel-skill.ts, .claude/skills/add-discord/SKILL.md @ 639577c3; channels branch src/channels/discord.ts @ 6ee516ad */}
...
<Steps>
  ...
  <Step title="Adapter install">
    The skill copies `src/channels/discord.ts` from the `channels` branch, registers it in the channel barrel, installs the pinned `@chat-adapter/discord` package, builds and tests, writes `DISCORD_BOT_TOKEN`, `DISCORD_APPLICATION_ID`, and `DISCORD_PUBLIC_KEY` to `.env`, and restarts the service.
  </Step>
