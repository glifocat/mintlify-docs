...
{/* verified-against: setup/install-signal-cli.sh, setup/signal-auth.ts, .claude/skills/add-signal/SKILL.md @ 4e83a0a0; channels branch src/channels/signal.ts @ 6ee516ad */}
...
Signal is offered in the first-run setup wizard, or add it later by running `/add-signal` in Claude Code. Both paths run the same `/add-signal` skill:
...
<Steps>
  ...
  <Step title="Adapter install">
    The `/add-signal` skill copies the adapter from the `channels` branch, registers it in the channel barrel, installs `qrcode` (only used to render the linking QR — the adapter itself has no npm dependencies), and builds. Idempotent: safe to re-run without re-linking.
  </Step>
  ...
  <Step title="Credentials and restart">
    The wizard reads the linked phone number back, writes `SIGNAL_ACCOUNT` to `.env`, and restarts the service so the adapter picks it up.
  </Step>
...
- **Group engagement** — group wirings default to mention engagement: the agent responds only when the linked account is @-tagged in the group. DMs always count as addressed to the agent. Override per wiring with a different [engage mode](/reference/ncl-cli#wirings).
- **Typing indicator** — shown while the agent works, in DMs only (Signal has no group typing).
- **Read receipts** — NanoClaw starts the managed daemon with `--send-read-receipts`, so senders see their messages marked read once the daemon receives them. An externally run daemon (`SIGNAL_MANAGE_DAEMON=false`) sends read receipts only if you add the flag to its command line yourself.
