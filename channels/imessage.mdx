...
{/* verified-against: .claude/skills/add-imessage/SKILL.md, .claude/skills/add-imessage/docs.md, setup/channels/imessage-configure.sh, scripts/photon-setup.ts, setup/auto.ts, setup/verify.ts @ 4446b5bd; channels branch src/channels/imessage.ts @ c6cdc212 */}
...
3. **Add yourself in the dashboard** — a manual step: in the Photon dashboard, add your phone number as a user and accept the invite that arrives on your phone. This opt-in can only be granted through the dashboard invite flow; the wizard polls (10 minutes by default) until it lands rather than reporting success early.
4. **Credentials written** — `PHOTON_PROJECT_ID` and `PHOTON_PROJECT_SECRET` land in `.env`; the assigned iMessage number is recorded in `data/photon-auth.json`.
...
- **`Target not allowed for this project` (hosted)** — the recipient hasn't opted in to the line. Add the number as a user in the Photon dashboard, accept the invite on that phone, and text the line once.
...
- **"Couldn't finish connecting" at the end of setup** — the welcome message didn't go out. Local: double-check Full Disk Access. Hosted: confirm you accepted the dashboard invite and have texted the line once.
