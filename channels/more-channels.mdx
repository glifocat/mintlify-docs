...
{/* verified-against: .claude/skills/add-{whatsapp-cloud,gchat,github,linear,matrix,webex,wechat,deltachat,emacs,resend}/SKILL.md, src/webhook-server.ts @ 4e83a0a0; channels branch src/channels/{whatsapp-cloud,gchat,github,linear,matrix,webex,wechat,deltachat,emacs,resend}.ts @ 6ee516ad; WhatsApp Cloud instance route: src/channels/chat-sdk-bridge.ts @ b76fcb3d; channels branch src/channels/whatsapp-cloud.ts @ 6d5c1d08 */}
...
- **Transport:** Chat SDK bridge; webhook at `/webhook/whatsapp-cloud`, subscribed to the `messages` field
...
- [Mattermost](/channels/mattermost): offered by the setup wizard and documented on its own page (server discovery, bot account, authenticated card callbacks)
