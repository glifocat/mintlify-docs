...
{/* verified-against: .claude/skills/{add-dial,add-dial-number,add-dial-tool}/SKILL.md, setup/pair-dial.ts, setup/auto.ts @ 475ba766; channels branch src/channels/dial.ts + dial-pairing.ts @ a45fab71 */}
...
<Steps>
  ...
  <Step title="Add phone superpowers (optional)">
    The wizard offers to run `/add-dial-tool`, which puts the `dial` CLI inside the agents you choose so they can text and call from any channel. See [The Dial tool](#the-dial-tool-sms-and-calls-from-inside-the-sandbox) below. Saying no keeps the channel working without it.
  </Step>
</Steps>
...
- **Replies come from the wrong number** (multi-number accounts) — the installed adapter predates multi-line routing. `/add-dial-number` checks for this and tells you to refresh `dial.ts` from the `channels` branch.
