...
{/* verified-against: src/channels/chat-sdk-bridge.ts, src/webhook-server.ts, setup/channels/teams-manifest-build.ts, .claude/skills/add-teams/SKILL.md @ 4e83a0a0; channels branch src/channels/teams.ts @ 6ee516ad */}
...
<Steps>
  ...
  <Step title="Adapter install">
    The `/add-teams` skill copies the adapter from the `channels` branch, installs the pinned package, builds, writes `TEAMS_APP_ID`, `TEAMS_APP_PASSWORD`, `TEAMS_APP_TYPE` (and `TEAMS_APP_TENANT_ID` for single tenant) to `.env` (synced to `data/env/env`), and restarts the service.
  </Step>
...
- **Webhook delivery** — the bridge registers Teams on the shared webhook server (port 3000, configurable via `WEBHOOK_PORT`), which routes `/webhook/teams` to the adapter. The public URL must stay reachable — if your tunnel dies, the bot silently stops hearing messages.
  <Warning>
  The setup wizard and the `/add-teams` skill currently print `/api/webhooks/teams` as the messaging endpoint path, but `src/webhook-server.ts` routes `/webhook/{routingPath}` (which defaults to the adapter name, i.e. `/webhook/teams`). If your bot never receives messages and the webhook server logs 404s, point the Azure messaging endpoint at `https://<your-public-host>/webhook/teams`.
  </Warning>
