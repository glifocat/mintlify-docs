...
keywords: ["channels", "adapters", "channel registry", "chat sdk", "webhook", "add-telegram", "add-discord", "add-slack", "add-mattermost", "add-dial", "manage-channels"]
---

{/* verified-against: src/channels/{adapter,channel-registry,channel-defaults,index,chat-sdk-bridge,cli}.ts, src/webhook-server.ts, setup/auto.ts, setup/channels/{run-channel-skill,slack-auto-register,slack-auto}.ts, .claude/skills/{manage-channels,add-telegram}/SKILL.md @ 53ea1e8b; channels branch src/channels/, src/modules/{slack-room-membership,canvas-actions,slack-onboarding,slack-agent-flow}/, .claude/skills/{add-slack,slack-agent-flow,slack-a2a-rooms}/SKILL.md @ d69c9c5d; Dial: .claude/skills/add-dial/SKILL.md, setup/auto.ts picker @ 475ba766, channels branch src/channels/dial.ts @ a45fab71; Mattermost: .claude/skills/add-mattermost/SKILL.md, setup/channels/initial-setup.ts @ 294ef2ae, channels branch src/channels/mattermost.ts @ 4a263658; rerun behavior and Slack companion location: .claude/skills/{add-telegram,add-whatsapp,add-slack}/SKILL.md, setup/channels/{slack-auto-register,run-channel-skill}.ts @ b76fcb3d; registry browser channel: channels branch src/channels/{index,local-web}.ts @ 6d5c1d08 */}
...
*/}
<img className="block dark:hidden" src="/images/diagrams/channels-overview-light.svg" alt="Trunk ships the channel infrastructure and the built-in CLI channel; platform adapters live in the channels-branch catalog and are installed on demand with /add-channel skills; a wiring connects each messaging group to an agent group" />
<img className="hidden dark:block" src="/images/diagrams/channels-overview-dark.svg" alt="Trunk ships the channel infrastructure and the built-in CLI channel; platform adapters live in the channels-branch catalog and are installed on demand with /add-channel skills; a wiring connects each messaging group to an agent group" />
...
- **Chat SDK adapters**: wrap a [Chat SDK](https://github.com/vercel/chat) platform adapter (the `chat` npm package and its `@chat-adapter/*` family) via the bridge in `src/channels/chat-sdk-bridge.ts`. The bridge handles mention detection, attachments, reply context, interactive question cards, and splitting long messages to fit platform limits. Discord, Slack, Telegram, Teams, and most others use this pattern. Mattermost is the variant case: it rides the same bridge, but its adapter is vendored source copied from the `channels` branch rather than an npm package.
- **Native adapters** — implement the `ChannelAdapter` interface directly. Used where no Chat SDK adapter fits: WhatsApp (Baileys), Signal (signal-cli), Dial (the `dial` CLI), WeChat, Delta Chat, Emacs, iMessage’s hosted backend (`spectrum-ts`), and the built-in CLI channel.
...
The install steps are designed to be re-run: adapter files are refreshed from the registry and existing imports are not duplicated. Credential steps can reuse an existing connection or offer another identity, depending on the channel. Re-running an `/add-<channel>` skill is how you pick up adapter improvements from the `channels` branch.

## Adapter catalog

Messaging platforms with install skills. "Setup wizard" means the first-run setup offers the channel and runs its `/add-<channel>` skill for you; the others are installed by running the skill directly.
...
| Slack | Yes | `/add-slack` | Managed or manual app setup; Socket Mode or webhook delivery. Per-agent bot identities, shared rooms, and canvases come from the separate `/slack-a2a-rooms` and `/slack-agent-flow` skills |
| Mattermost | Yes | `/add-mattermost` | Vendored Chat SDK adapter; WebSocket inbound (no public URL for chat); reuses your server or creates a local evaluation server; authenticated card callbacks |
| Signal | Yes | `/add-signal` | Native; requires signal-cli with a linked account |
| iMessage | Yes | `/add-imessage` | Local backend: Chat SDK on this Mac (Full Disk Access); hosted backend: native adapter via photon.codes (`spectrum-ts`) |
| Microsoft Teams | Yes | `/add-teams` | Chat SDK; webhook-based; thread-based |
| Dial | Yes | `/add-dial` | Native; a real phone number for SMS and AI voice calls through the `dial` CLI; 6-digit SMS pairing; owner-only or public line; `/add-dial-number` adds more lines |
...
The `channels` branch also contains a `local-web` browser channel. It listens on `127.0.0.1:3210` by default and protects conversation data and actions with a local token. It is not included in the main setup wizard and has no `/add-local-web` install skill on trunk.
...
<CardGroup cols={3}>
  ...
  <Card title="Slack" icon="hashtag" href="/channels/slack">
    Managed apps, agent rooms, canvases
  </Card>
  <Card title="Mattermost" icon="server" href="/channels/mattermost">
    Your server or a local evaluation stack
  </Card>
  ...
  <Card title="Dial" icon="phone" href="/channels/dial">
    Real phone number: SMS and voice
  </Card>
