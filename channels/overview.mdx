...
- **Native adapters** — implement the `ChannelAdapter` interface directly. Used where no Chat SDK adapter fits: WhatsApp (Baileys), Signal (signal-cli), WeChat, Delta Chat, Emacs, iMessage’s hosted backend (`spectrum-ts`), and the built-in CLI channel.
