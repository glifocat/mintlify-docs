...
{/* verified-against: src/channels/cli.ts, src/channels/index.ts, scripts/chat.ts, scripts/init-cli-agent.ts, setup/auto.ts, src/cli/socket-server.ts, package.json @ 639577c3; standing instructions, ping folder and named-instance routing: setup/lib/agent-ping.ts, src/group-init.ts, src/channels/cli.ts @ b76fcb3d */}
...
This creates the synthetic `cli:local` user, an agent group with initial standing instructions in `instructions.prepend.md`, and a "Local CLI" messaging group wired to it. It's idempotent and safe to run alongside the live service. The setup wizard runs the same script during its `cli-agent` step — first with a scratch `ping_test` agent to confirm the sandbox responds (cleaned up afterwards), and again with a permanent agent if you choose "chat from the terminal".
...
The wire format is one JSON object per line. A plain chat message is `{"text": "..."}` in, `{"text": "..."}` out. Clients holding the socket (i.e., the owner) can also send routed messages with a `to` address (`channelType`, `platformId`, optional `threadId` and `instance`) to inject a message into any wired channel. Set `instance` when targeting a named bot; otherwise it defaults to the channel type. This is how admin transports like the bootstrap script target other platforms. The CLI channel itself sets `supportsThreads: false`; the `threadId` in a routed address targets threads on the *destination* channel, not CLI sessions.
