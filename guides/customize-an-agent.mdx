...
description: "Hands-on tutorial: change an agent's personality through instructions.prepend.md, swap its model and effort with ncl, and verify the changes landed."
keywords: ["customize", "instructions.prepend.md", "personality", "model", "effort", "ncl groups config update", "self-customize", "/customize"]
---

{/* verified-against: src/cli/resources/groups.ts, container/agent-runner/src/providers/types.ts, container/skills/self-customize/SKILL.md, .claude/skills/customize/SKILL.md @ 639577c3; project document: src/project-doc-compose.ts, src/group-persona.ts, src/container-runner.ts @ 294ef2ae; model fallback: src/config.ts, src/container-config.ts @ b76fcb3d */}
...
One file you will **not** edit: `groups/<folder>/CLAUDE.md`. The host regenerates it on every container spawn and re-mounts it read-only on top of the group dir. Every instruction source is read on the host and inlined into that one flat file, so anything you write there gets clobbered. `instructions.prepend.md` is the file that's yours (and the agent's); durable facts the agent accumulates over time live alongside it in the [`memory/` tree](/concepts/agent-memory).
...
```markdown groups/scout/instructions.prepend.md
## Style
...
- `--model` takes an alias (`sonnet`, `opus`, `haiku`) or a full model ID. When the group has no model, `NANOCLAW_DEFAULT_MODEL` supplies the install default; if that is also unset, the provider SDK chooses.
...
It's a header and full text. The composed document is one flat file, the composer emits no `@`-imports, and a test pins that they never come back. Your `instructions.prepend.md` is inlined verbatim as the first section, `# Persona`, so it opens the project memory the agent loads, ahead of the shared base (`container/CLAUDE.md`), the module manuals, the resident skill prose, and any instructions from MCP servers you added. The persona section is never dropped, even when the document hits its size cap. Editing the composed `CLAUDE.md` is pointless because the host rebuilds it from those sources on every spawn.
