...
<Steps>
  ...
  <Step title="Finish the install">
    Let the installer finish. It pulls the image by its pinned digest, refuses it if the architecture or the agent-runner lock label is wrong, and tags it as this install's sandbox image, so your agents are ready to run on it.
