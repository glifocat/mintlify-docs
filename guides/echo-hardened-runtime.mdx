...
{/* verified-against: setup/auto.ts, setup/registry-login.ts, setup/registry-login.sh, setup/registry.ts, setup/lib/registry-state.ts, container/Dockerfile, container/pull.sh, container/build.sh, setup/verify.ts, docs/hardened-image.md, versions.json @ d9745d09 (feat/hardened-agent-image, PRE-MERGE — re-verify and restamp at the merge SHA before this page ships; the branch force-pushes, diff with three dots). */}
...
- **No local build.** Fetching replaces the 3–10 minute local build with a single download of roughly 800 MB. On a slow connection the local build can be the quicker path, and both stay available.
