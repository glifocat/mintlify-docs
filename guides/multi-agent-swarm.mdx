...
*/}
<img className="block dark:hidden" src="/images/diagrams/multi-agent-swarm-light.svg" alt="Worker, manager, and supervisor agent groups connected by directed agent_destinations rows; a send with no row is rejected by the host" />
<img className="hidden dark:block" src="/images/diagrams/multi-agent-swarm-dark.svg" alt="Worker, manager, and supervisor agent groups connected by directed agent_destinations rows; a send with no row is rejected by the host" />
...
2. Scaffolds `groups/researcher/` and seeds the **`instructions` text into the new agent's `instructions.prepend.md`** — its standing instructions, composed into the top of the child's `CLAUDE.md` at spawn (the same on every provider now). That's the agent's persistent role and personality, editable later like any agent's. A default `container_configs` row comes with it, and the child inherits its creator's provider.
...
4. Projects the new destination into the parent's running container immediately, then notifies it that researcher was created and can now receive `send_message` calls.
...
- **Supervisor pings you**: wire the supervisor to your DM (`ncl wirings create --messaging-group-id <your-dm-id> --agent-group-id <supervisor-id> ...`) so it receives your messages — and can reply, since an agent may always answer its origin chat. The command also creates the channel destination in the same transaction and projects it into live sessions, so a *proactive* send works immediately. The auto-created local name is the normalized messaging-group name (with a `<channel_type>-<id-prefix>` fallback). Add an explicit edge only when you want a custom local name:
  ...
  Now the supervisor can message you unprompted — and use `ask_user_question` to put actual buttons under a decision. (Edge case: the destination is only auto-created when the agent-to-agent module is installed — but without that module the delivery ACL is skipped too.)
- **One conversation per task**: wire a worker to a thread-capable channel (Discord, Slack) with `--session-mode per-thread --threads true` and every thread gets its own provider conversation. Sibling sessions in the same conversation can still receive non-triggering background context through cross-session echoes. On server channels this is [forced anyway](/concepts/entity-model); the flag matters for DMs.
...
docker ps --filter label=nanoclaw-install            # the containers themselves
```
...
- **Edits to destinations outside `ncl` don't propagate live.** A running container serves a projection of its destinations, refreshed on every wake and by `ncl destinations add`/`remove` and `ncl wirings create` — but direct DB writes leave it stale until the next wake.
...
This cascades through the central DB in one transaction: sessions, destination rows in **both directions** (its own and every edge pointing at it), pending approvals, wirings, memberships, and the container config. It does **not** kill a running container or delete `groups/<folder>/` and `data/v2-sessions/<group-id>/` — stop the container and remove those by hand if you want the workspace gone. A later bare create refuses to reuse a leftover group folder without an owning database row, so move it or choose a different folder name.
