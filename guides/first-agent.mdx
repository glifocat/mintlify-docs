...
{/* verified-against: src/cli/resources/{groups,wirings,messaging-groups,sessions}.ts, src/cli/crud.ts, src/router.ts, src/channels/cli.ts, scripts/chat.ts, scripts/init-cli-agent.ts, src/group-init.ts, src/container-runner.ts, setup/auto.ts @ 639577c3; template behavior @ 20d61af; folder reuse and runtime names: src/cli/resources/groups.ts, src/drivers/docker-driver.ts @ b76fcb3d */}
...
# For just this install: --filter label=nanoclaw-install=<slug>
```
