...
{/* verified-against: src/cli/resources/{groups,wirings,messaging-groups,sessions}.ts, src/cli/crud.ts, src/router.ts, src/channels/cli.ts, scripts/chat.ts, scripts/init-cli-agent.ts, src/group-init.ts, src/container-runner.ts, setup/auto.ts @ 639577c3 */}
...
Now check the filesystem: `ls groups/` — the `scout/` folder is already there. Create does more than insert the DB row: it runs `initGroupFilesystem()` inline, so the folder, an empty `instructions.prepend.md`, and the container-config row all exist before any message arrives. Re-running create with the same `--folder` is safe — it returns the existing row. What does *not* exist yet: the composed `CLAUDE.md` (generated at first container spawn) and any session state. An agent that has never been messaged still costs nothing at runtime — no container runs for it.
...
```bash
ls groups/scout/
# CLAUDE.md  instructions.prepend.md
```

`CLAUDE.md` is the composed instructions file the host regenerates on every spawn — don't edit it. `instructions.prepend.md` is the agent's editable standing instructions, composed into the top of `CLAUDE.md` at every spawn. Groups created via `ncl` start with it **empty** — edit it to give Scout a personality and standing instructions (see [customize an agent](/guides/customize-an-agent)). Durable facts the agent accumulates over time live in its `memory/` tree.
