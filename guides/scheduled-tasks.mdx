...
{/* verified-against: src/cli/resources/tasks.ts, container/agent-runner/src/mcp-tools/scheduling.instructions.md, src/modules/scheduling/{create,db,recurrence,run-log}.ts, container/agent-runner/src/scheduling/task-script.ts, container/agent-runner/src/poll-loop.ts, container/agent-runner/src/timezone.ts, src/host-sweep.ts, src/session-manager.ts, src/config.ts, src/container-runner.ts, src/cli/resources/groups.ts @ 639577c3; template behavior @ 20d61af; reconciliation and task deletion: src/host-sweep.ts, src/reconcile-session.ts, src/cli/resources/tasks.ts, src/mailbox/sqlite/tasks.ts @ b76fcb3d */}
...
<Note>
Agents stamped from a [template](/templates/overview) may arrive with recurring tasks already defined. They are created **paused** — list them with `ncl tasks list --group <agent-group-id> --status paused` and enable the ones you want with `ncl tasks resume <task-id>`.
</Note>
...
There is no separate per-task scheduler. The [host sweep](/concepts/architecture#what-runs-when) periodically queues active sessions for reconciliation, waiting 60 seconds after the previous tick's work drains. Message writes and container-exit events can trigger earlier checks. Reconciliation counts pending rows with `process_after <= now` and wakes the task session's container if one isn't already running. The agent's poll loop then picks the task up like any inbound message and executes the prompt.
...
| `ncl tasks update <id>` | Edits the live occurrence (matches by id *or* series id). Omitted fields are untouched; an empty `--prompt` is rejected. `--recurrence null` clears the schedule (the task becomes one-shot) and `--script null` removes the gate. |
...
| `ncl tasks delete <id>` | For modern isolated tasks, delete the whole task session, mailbox history, attachments, heartbeat, and run log. Refused while its container is running. Legacy shared sessions keep their mailbox and lose only this series and its run log. |
...
The result: naive timestamps the agent passes (`2026-06-11T09:00:00`, no offset) and **cron expressions** — both at schedule time and when the host sweep computes the next recurrence — are interpreted in the task's timezone: the owning group's override when one is set, otherwise that instance value (`src/modules/scheduling/create.ts`). To pin the install default:
