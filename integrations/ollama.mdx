...
The `/add-ollama-tool` skill adds a stdio-based MCP server inside the agent container. The MCP server exposes core inference tools (always available) and optional model management tools.

### Core tools

| Tool | Description |
|------|-------------|
| `ollama_list_models` | Lists all locally installed Ollama models with name, size, and family |
| `ollama_generate` | Sends a prompt to a specified model and returns the response |

### Model management tools (opt-in)

These tools are available when `OLLAMA_ADMIN_TOOLS=true` is set in `.env`:
...
To enable model management tools (pull, delete, inspect, list running), add to `.env`:

```bash
OLLAMA_ADMIN_TOOLS=true
```

Without this variable, the agent can only list installed models and generate responses — you manage models yourself on the host.
...
<Warning>
When using custom endpoints, the secret injection layer (OneCLI gateway in v1.2.22+, or the credential proxy in earlier versions) still intercepts container API requests. Ensure the endpoint is reachable from the host.
</Warning>
