...
- The base NanoClaw codebase stays small (~34.9k tokens)
...
Telegram ships on `main` as a core channel. Other channels (WhatsApp, Discord, Slack, Gmail) live in **separate fork repositories** and are added via skills.

| Channel | Location | Library | How to add |
|---------|----------|---------|------------|
| Telegram | `main` (core) | grammy | Available by default |
| WhatsApp | `qwibitai/nanoclaw-whatsapp` | @whiskeysockets/baileys | `/add-whatsapp` |
| Discord | `qwibitai/nanoclaw-discord` | discord.js | `/add-discord` |
| Slack | `qwibitai/nanoclaw-slack` | @slack/bolt (Socket Mode) | `/add-slack` |
| Gmail | `qwibitai/nanoclaw-gmail` | googleapis | `/add-gmail` |
...
Docker sandbox forks are also maintained as part of the ecosystem:

| Fork repo | Purpose |
|-----------|---------|
| `qwibitai/nanoclaw-docker-sandboxes` | Docker sandbox setup |
| `qwibitai/nanoclaw-docker-sandbox` | Single Docker sandbox |
| `qwibitai/nanoclaw-docker-sandbox-windows` | Docker sandbox for Windows |
...
<Info>
Telegram is the only channel bundled on `main`. All other channels are added by merging from the appropriate fork repository, keeping the base codebase minimal.
</Info>
...
| Channel | Command | Source | Library |
|---------|---------|--------|---------|
| Telegram | Built-in | `main` (core) | grammy |
| WhatsApp | `/add-whatsapp` | `nanoclaw-whatsapp` fork | @whiskeysockets/baileys |
| Discord | `/add-discord` | `nanoclaw-discord` fork | discord.js |
| Slack | `/add-slack` | `nanoclaw-slack` fork | @slack/bolt (Socket Mode) |
| Gmail | `/add-gmail` | `nanoclaw-gmail` fork | googleapis |
...
Channel forks also have a sync workflow that pulls upstream `main` and auto-resolves common conflicts:

- **`package-lock.json`**, **`package.json`**, **`repo-tokens/badge.svg`**, **`.github/workflows/*`**: always take upstream's version
- **`.env.example`**: keep the fork's version (preserves channel-specific environment variables)
- Non-trivial conflicts abort the merge and create an issue
