...
- **`/use-native-credential-proxy`** — Replace OneCLI gateway with built-in `.env`-based credential proxy (Docker only — Apple Container already includes credential proxy code, so this skill must not be applied on top of `/convert-to-apple-container`)
