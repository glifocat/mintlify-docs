...
Telegram is included on `main` as a core channel. Other channels are installed via the [skills system](/integrations/skills-system). During `/setup`, you choose which platforms to connect — pick one or many:

<CardGroup cols={2}>
  <Card title="Telegram" icon="paper-plane" href="/integrations/telegram">
    Core channel — available by default. Uses the grammy framework.
  </Card>
...
Telegram is available by default — just configure a bot token during `/setup`. For other channels, use the corresponding skill command in Claude Code:
...
Telegram is the default channel and ships on `main`. You can add additional channels during `/setup` or later. All channels are peers — you can run one or several simultaneously.
