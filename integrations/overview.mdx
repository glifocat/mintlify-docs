...
| `/add-signal` | Signal | Wired into `bash nanoclaw.sh`; requires `signal-cli` |
...
`agent_provider` is stored both on `agent_groups` (group-wide default) and `sessions` (per-session override) in the central database. When the host spawns a container it resolves the provider as `sessions.agent_provider` → `agent_groups.agent_provider` → `container.json` `provider` → `claude`, letting a single session branch to a different provider without touching the group default.
