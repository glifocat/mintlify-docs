...
|-------|----------|-------|
| `/add-signal` | Signal | signal-cli bridge |
...
Provider resolution follows a strict precedence: `sessions.agent_provider` overrides `agent_groups.agent_provider`, which overrides the `provider` field in the group's `container.json`, which falls back to `claude`. Empty strings are treated as unset. This lets you override the provider for a single session without touching the group's default.
