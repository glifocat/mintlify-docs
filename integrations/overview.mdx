...
Each agent group can run a different provider — mix and match in the same install. Set `"provider": "<name>"` in the group's `container.json`; the in-container runner reads from there. The `agent_groups.agent_provider` and `sessions.agent_provider` DB columns drive host-side contribution only (per-session mounts, env passthrough) and do not propagate into `container.json` at spawn time.
