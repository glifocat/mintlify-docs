...
description: Telegram is NanoClaw's core channel, available on main by default
tag: "UPDATED"
---
...
Telegram is NanoClaw's core channel, bundled on `main` and available by default. It can run alongside other channels added via skills.
...
Telegram ships on `main` — no skill merge is needed. During `/setup`, you configure your bot token and register chats:
...
<Note>
Telegram was previously a skill-based channel (`/add-telegram`). It was promoted to a core channel and now ships on `main` with the `grammy` library included in the base dependencies.
</Note>
...
Since Telegram is a core channel on `main`, it cannot be removed via a skill revert. To disable it, remove the `TELEGRAM_BOT_TOKEN` variable from `.env` — the channel will emit a warning at startup and be skipped.

To fully remove the Telegram code, delete `src/channels/telegram.ts` and remove its import from `src/channels/index.ts`, then rebuild:
...
<Warning>
Removing Telegram from the source may cause merge conflicts when updating from upstream. Disabling via the environment variable is the recommended approach.
</Warning>
