...
Set `LOG_LEVEL=debug` (see [Configuration](/operate/configuration#applying-changes)) to also see resolved mount configurations and streamed container stderr tagged with the container name. Containers run with `--rm`, so the host log is the only place container output survives.
...
<Steps>
  <Step title="Did the router accept it?">
    ...
    | `unknown_sender_decline_notify` | Sender not recognized; in a DM the bot politely declined and notified an owner or admin | Add the sender if they should have access, or change the policy — repeated declines are deduplicated for 24 hours |
  </Step>
  <Step title="Did a container spawn?">
    ```bash
    ncl sessions list                      # container_status should be "running"
    grep 'Spawning session' logs/nanoclaw.log | tail -5
    docker ps --filter label=nanoclaw-install --format '{{.Names}} {{.Status}}'
    ```
...
- **Scheduled or retried messages wait for reconciliation**: a periodic resync checks active sessions roughly once a minute, and message writes or container-exit events can trigger earlier checks. Queue work and container cold starts add latency; one minute is not a strict upper bound. Look for `Waking container for due messages` in the log.
...
## Recovering a misbehaving session

Start with `/debug` in Claude Code and identify the session with `ncl sessions list`. Do not delete a session directory while its host or container is using it: a running container may still hold the mounted databases, and the host also tracks durable runtime state. Back up the install before any destructive recovery. Restarting a group with `ncl groups restart --id <group-id>` replaces its running containers while preserving conversation data.

## Template agent replies 401 or 403

Expected on first use, not a fault. A [template](/templates/overview) ships no credentials, so the first call to a service the [vault](/operate/credentials) has no key for fails. The agent replies with a prefilled connect link for that host — open it, paste the key, and ask it to retry.

The template's own `README.md` lists the services it expects and how to connect them.

## Template task never fires

Template tasks are created **paused** on purpose, so stamping never starts background work without consent. Nothing fires until you resume them:
...
If a task is already resumed and still quiet, it's a normal scheduling problem rather than a template one — see [Scheduled tasks](/guides/scheduled-tasks).

## Restamping reports "Nothing to apply"

Restamping reads only your **local** `templates/` copy, and nothing refreshes it for you. Update the local folder first — edit it if you authored it, or re-download the registry copy — then re-run. See [Updating a stamped agent](/templates/updating).
