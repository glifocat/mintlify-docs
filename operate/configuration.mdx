...
{/* verified-against: src/config.ts, src/env.ts, src/log.ts, src/webhook-server.ts, src/egress-lockdown.ts, src/host-sweep.ts, src/providers/claude.ts, src/container-config.ts, src/db/container-configs.ts, setup/set-env.ts, setup/service.ts, setup/container.ts, container/pull.sh @ 639577c3; instance model and runtime settings: src/config.ts, src/container-config.ts, src/drivers/index.ts, src/gateway-providers/index.ts, container/agent-runner/src/providers/claude.ts @ b76fcb3d */}
...
- **The process-only settings listed below are read from `process.env` only.** The launchd plist, systemd unit, and nohup wrapper that setup generates set just `PATH` and `HOME` — they do **not** source `.env`. Putting these variables in `.env` has no effect; you have to set them in the service environment (see [Applying changes](#applying-changes)).
...
| `DEFAULT_AGENT_PROVIDER` | `claude` | Provider stamped on **new** agent groups at creation. Existing groups are never touched, and the per-group `provider` config still wins. |
| `NANOCLAW_DEFAULT_MODEL` | provider SDK default | Model used at the next spawn for groups without their own model. Unlike the provider default, this is not stamped at group creation. |
| `NANOCLAW_FAST_MODE` | `false` | Enables the faster, higher-priced serving tier for the Claude provider. Only `1` or `true` enables it, case-insensitively. |
| `NANOCLAW_RUNTIME_DRIVER` | `docker` | Selects an installed session driver. Docker is the only driver shipped here; an unregistered value fails. |
| `NANOCLAW_GATEWAY_PROVIDER` | `onecli` | Selects an installed credential-gateway provider. An unregistered value fails. |
| `NANOCLAW_SESSION_MATERIAL_ROOT` | `data/session-materials` | Host directory admitted for session identity material. |
...
| `CONTAINER_CPU_LIMIT` | _unset_ (unbounded) | Opt-in CPU cap per container, passed to `docker create --cpus` (e.g. `2`). Can be set in `.env` or the process environment. |
| `CONTAINER_MEMORY_LIMIT` | _unset_ (unbounded) | Opt-in memory cap per container, passed to `docker create --memory` (e.g. `8g`). Hard cap only on a swapless host. Can be set in `.env` or the process environment. |
...
Host-only settings need the service environment or an exported value in your dev shell. Setup also reads `NANOCLAW_HARDENED_IMAGE` and `NANOCLAW_AGENT_IMAGE_REF` from `.env`, with the process environment taking precedence.
...
| `NANOCLAW_HARDENED_IMAGE` | `false` | Set `true` to fetch the pre-built [Echo hardened image](/guides/echo-hardened-runtime) at the next build/install step instead of building locally (`setup/container.ts:286`). |
| `NANOCLAW_AGENT_IMAGE_REF` | committed `agent-image` pin in `versions.json` | Override which pullable image reference to fetch (`setup/container.ts:309`). |
| `NANOCLAW_ALLOW_UNLABELED_IMAGE` | `false` (unset) | Accept a pulled image with no `dev.nanoclaw.agent-runner-lock-sha256` label instead of refusing it (`container/pull.sh:153`). See [Hardening](/operate/hardening#hardened-image-option). |
...
|---|---|
| `provider` | Which agent CLI runs in the container (e.g. Claude Code, OpenCode). New groups start with the `DEFAULT_AGENT_PROVIDER` value. |
| `model`, `effort` | Model and reasoning effort passed to the provider. |
| `assistant_name` | Per-group assistant name override. |
| `max_messages_per_prompt` | Maximum inbound messages supplied in one provider turn for this group. |
...
A host restart can adopt containers that are already running. After changing `NANOCLAW_DEFAULT_MODEL`, `NANOCLAW_FAST_MODE`, or another spawn-time setting, also run `ncl groups restart --id <group-id>` for each affected group to apply it.
