...
{/* verified-against: setup/uninstall/{flow,scan,plan,remove,onecli-agents}.ts, setup/lib/setup-config.ts, setup/auto.ts, uninstall.sh, nanoclaw.sh, src/install-slug.ts @ 639577c3; identity and orphan deletion: src/install-slug.ts, setup/uninstall/flow.ts, setup/uninstall/remove.ts, src/mailbox/sqlite/schema.ts @ b76fcb3d */}
...
| **2) App data, logs & secrets** | Runtime data, including the central database and per-session message history, logs, build files (`dist/`, `node_modules/`), and the active `.env`. Removing this erases stored conversations; the `.env` is backed up first. |
