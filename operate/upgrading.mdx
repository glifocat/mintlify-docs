...
{/* verified-against: .claude/skills/update-nanoclaw/SKILL.md, .claude/skills/update-skills/SKILL.md, .claude/skills/migrate-nanoclaw/SKILL.md, versions.json, scripts/upgrade-state.ts, src/upgrade-state.ts @ 639577c3 */}
...
1. **Self-refresh.** It fetches upstream and reads the upstream copy of its own SKILL.md (`git show`, no working-tree change); if that version differs, it follows the newer instructions — the merge later brings them into the checkout.
...
7. **Validation.** Runs `pnpm run build` and `pnpm test`. If `container/` files changed — or, on an install that pulls the [hardened image](/guides/echo-hardened-runtime), the `agent-image` pin in `versions.json` moved — it also refreshes the container image: `./container/build.sh` on a local-build install, `./container/build.sh pull` on a pinned one. The bare form exits `3` on a pinned install rather than silently replacing the pulled image with a local build.
...
9. **Breaking changes.** Scans the new `CHANGELOG.md` entries for `[BREAKING]` lines. Each names either a migration skill (`Run /<skill-name> to …`) or a migration guide (`Follow [the migration guide](docs/<guide>.md)`). The skill lists both kinds and offers to run the ones you pick — for a guide, it reads the referenced page, summarizes it, and executes its fix and verification steps. Unresolved items from either kind land in the same follow-up list.
...
6. **Container rebuild.** If a re-applied skill changed files under `container/` (e.g. a provider's in-container runtime), the skill refreshes the agent image — `./container/build.sh` on a local-build install, `pull` on a pinned one — until then new sessions keep running the old image, so the refresh is what makes the change live, not the file copy. A channel-adapter-only refresh skips this.
...
- **Container rebuild.** If `container/` changed, the image was rebuilt (or re-pulled, on a hardened-image install) — running agent containers keep the old image until their session ends; new sessions use the new one.
