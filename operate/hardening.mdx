...
By default your machine builds the agent sandbox image locally. You can opt in to a pre-built, patched image instead — see [Get started with the Echo hardened runtime](/guides/echo-hardened-runtime).
...
| Value | Meaning |
|---|---|
| `local` | Built on this machine from the public base. The default. |
...
A second label, `dev.nanoclaw.agent-runner-lock-sha256`, records which `container/agent-runner/bun.lock` the baked `node_modules` was built from. `container/pull.sh` checks it before retagging, so a pulled image whose lock doesn't match this checkout fails loudly instead of surfacing as a missing module inside a container.
