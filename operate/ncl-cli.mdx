...
`create` also takes `--template <ref>` to stamp the new group from a [template](/templates/overview) — a ref relative to your local `templates/` directory, e.g. `sales/sdr`. When a group already carries that template's plugin, the same command shows an in-place [update plan](/templates/updating) instead; `--yes` applies it and `--new` forces a second agent.
