...
Templates ship **no secrets**. Any MCP server that needs an API key gets one from the [credentials proxy](/operate/credentials), which injects it into outbound HTTPS calls at request time — matched by API host.
