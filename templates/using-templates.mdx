...
{/* verified-against: src/templates/{create-agent,local-dir,parse}.ts, src/cli/resources/{groups,tasks}.ts @ nanocoai/nanoclaw#3220 1293ec56; setup/{templates,auto}.ts, docs/templates.md @ nanocoai/nanoclaw#2909 ed19f8ea */}
...
<Note>
Installs that already have a channel-wired agent don't see the picker. To stamp a template on one, use the CLI below or pass an explicit template path.
</Note>

**If setup failed partway through**, your pick is remembered in `.env`, so a rerun asks whether to continue with the same template. If the agent was already stamped, setup shows an update-in-place confirmation instead — declining leaves the existing agent untouched. See [Updating a stamped agent](/templates/updating) for what that update covers.
