...
Each line names a surface, the action, and whether the live copy has drifted:

```text
plugin       plugins/sdr                    update      replaced wholesale from the template
persona      instructions.prepend.md        update      CUSTOMIZED
skill        sdr-agent                      update
skill        old-play                       remove
context      additional_context/pricing.md  create
mcp-server   hubspot                        unchanged
mcp-server   exa                            skip        a server you added has this name; the plugin version was not installed
task         morning-digest                 update
```

Actions are `create`, `update`, `remove`, `skip`, and `unchanged`. Surfaces are `plugin`, `persona`, `context`, `skill`, `mcp-server`, and `task`.
