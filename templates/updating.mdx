...
Restamping only works against the **same plugin name**. To move an agent to a different template, create a new agent.
...
Applying swaps the status line for `Restamp applied. Run ncl groups restart --id <id> for skill and MCP changes to take effect.` — or `Nothing to apply — the group already matches the template.` when every surface was already current.
