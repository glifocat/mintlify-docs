...
Check the create response for a `templateReport` array. Any entry there means a component was skipped or ignored — a non-conforming skill, an unsupported MCP transport, an unknown manifest field. CI won't catch these — it validates only `$schema` and `name` — so fix them before submitting; they are never silently stripped, and reviewers will see them too.
