...
<Note>
Templates use the [Agent Plugins 1.0.0](https://agent-plugins.org) format. Templates written for the earlier layout (a bare `context/instructions.md` marker and `.mcp.json`) are no longer read — see [migrating from the pre-plugin layout](/templates/authoring#migrating-from-the-pre-plugin-layout).
</Note>
...
Only `plugin.json` is required — it is the manifest and the discovery marker. Everything else is optional and defaults sensibly. See [Authoring a template](/templates/authoring) for the full anatomy and [the template format reference](/reference/template-format) for field-level rules.
