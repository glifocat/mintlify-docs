...
{/* verified-against: src/templates/{parse,manifest,extension,mcp,skills,tasks,plugin-dir,create-agent,local-dir}.ts, src/cli/resources/groups.ts, docs/templates.md @ nanocoai/nanoclaw#3220 f8563abe; setup/{templates,auto}.ts @ nanocoai/nanoclaw#2909 ed19f8ea; nanocoai/nanoclaw-templates @ 3c7966a7 */}
...
<Note>
Templates use the [Agent Plugins 1.0.0](https://agent-plugins.org) format as of vX.Y.Z. Templates written for the earlier layout (a bare `context/instructions.md` marker and `.mcp.json`) are no longer read — see [migrating from the pre-plugin layout](/templates/building-and-submitting#migrating-from-the-pre-plugin-layout).
</Note>
...
Only `plugin.json` is required — it is the manifest and the discovery marker. Everything else is optional and defaults sensibly. See [Building and submitting a template](/templates/building-and-submitting) for the full anatomy and [the template format reference](/reference/template-format) for field-level rules.
...
| API keys and tokens | The [credentials proxy](/operate/credentials) injects them into outbound calls at request time. Credential-shaped values in a template are rejected at stamp time. |
...
| apt/npm packages | `ncl groups config add-package`, followed by `ncl groups restart --rebuild`. |

Because the provider is chosen separately, one template works on any provider.

## Where templates come from

Templates resolve **only from a local directory** — `templates/` at the project root by default (committed but shipped empty), or whatever `NANOCLAW_TEMPLATES_DIR` points at. There is no git URL, no `--source`, and no remote fetch at `ncl` time.
...
Then stamp it with its bare ref. See [Using a template](/templates/using-templates).
...
Field-level validation rules, limits, the runtime contract, and the report catalog.
</Card>
</Columns>
