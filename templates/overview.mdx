...
{/* verified-against: src/templates/{parse,manifest,extension,mcp,skills,tasks,plugin-dir,create-agent,local-dir}.ts, src/cli/resources/groups.ts, docs/templates.md @ nanocoai/nanoclaw#3220 1293ec56; setup/{templates,auto}.ts @ nanocoai/nanoclaw#2909 ed19f8ea; nanocoai/nanoclaw-templates @ 3c7966a7 */}
...
<Note>
Templates use the [Agent Plugins 1.0.0](https://agent-plugins.org) format. Templates written for the earlier layout (a bare `context/instructions.md` marker and `.mcp.json`) are no longer read — see [migrating from the pre-plugin layout](/templates/building-and-submitting#migrating-from-the-pre-plugin-layout).
</Note>
...
| apt/npm packages | `ncl groups config add-package --id <agent-group-id>`, followed by `ncl groups restart --id <agent-group-id> --rebuild`. |
