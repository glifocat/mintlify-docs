...
{/* verified-against: nanocoai/nanoclaw-templates {README.md,scripts/check-templates.mjs,.github/workflows/check-templates.yml} @ 3c7966a7, CONTRIBUTING.md @ nanocoai/nanoclaw-templates#20 95a4d4e, LICENSE @ 3c7966a7; src/templates/{mcp,manifest,tasks,skills}.ts @ nanocoai/nanoclaw#3220 1293ec56. Acceptance/ownership policy is product policy, not code-derived. */}
...
That path is also the ref people pass to `--template`: a template at `<category>/<template>/` is stamped with `--template <category>/<template>`. Templates nested deeper are not discovered.
