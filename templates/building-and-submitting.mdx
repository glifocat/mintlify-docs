...
- **The [credentials proxy](/operate/credentials) injects it at request time**, matched by API host. This is the normal path. The key never sits in `mcp.json`, the container environment, or chat context.
...
Never ship a shared key, a template-owner key, or any credential the contributor controls. The user's key goes in their own [credentials proxy](/operate/credentials) and stays there.
...
It walks every `<category>/<template>/` folder and reports all problems at once, exiting non-zero if any are found:
