...
<Update label="v1.2.35–v1.2.39 source sync" description="2026-03-27" tags={["Updated"]}>
  - **Product changelog**: Added entries for v1.2.35 through v1.2.39
  - **Token count**: Updated from ~41.3k to ~42.1k in introduction and skills system pages
  - **Mount security**: Documented colon rejection in container path validation (prevents Docker `-v` option injection) across security overview, security deep dive, and containers pages
  - **Mount allowlist caching**: Updated security deep dive to reflect that missing-file state is no longer permanently cached
</Update>
