...
<Update label="Templates: a new section for the Agent Plugins launch" description="2026-08-12" tags={["New"]}>
  Templates get their own documentation section, written against the Agent Plugins 1.0.0 format ([nanoclaw#3220](https://github.com/nanocoai/nanoclaw/pull/3220), [nanoclaw#2909](https://github.com/nanocoai/nanoclaw/pull/2909), [nanoclaw-templates#19](https://github.com/nanocoai/nanoclaw-templates/pull/19)). Templates shipped in v2.1.54 but had no coverage beyond one line in the `ncl` reference.

  - **New "Templates" nav group** — [overview](/templates/overview) (what a template is, why the format is a vendor-neutral standard, the catalog), [using a template](/templates/using-templates) (the wizard's library picker and the `ncl` path, ref resolution, post-stamp wiring and task activation), [updating a stamped agent](/templates/updating) (the restamp dry run, what resets versus what is never touched, CUSTOMIZED flags), and [building and submitting a template](/templates/building-and-submitting) (manifest, `mcp.json`, skills, persona, context extras, tasks, the pre-plugin migration, registry standards, `check-templates.mjs`, and the PR checklist).
  ...
  Verified against `nanocoai/nanoclaw#3220` at `f8563abe` and `#2909` at `ed19f8ea`, plus `nanocoai/nanoclaw-templates@3c7966a7` and [templates#20](https://github.com/nanocoai/nanoclaw-templates/pull/20) at `6b1fdd8b` for the registry standards. No product changelog entry yet — the release carrying this format isn't in upstream `package.json`.
</Update>
