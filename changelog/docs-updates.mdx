...
<Update label="Provider precedence + Signal channel" description="2026-04-24" tags={["Updated"]}>
  Synced with upstream v2.0.6 – v2.0.11 behavior changes.

  ## Updated
  - **`advanced/container-runtime`**: Clarified provider resolution order in the spawn lifecycle — `sessions.agent_provider` → `agent_groups.agent_provider` → `container.json` `provider` → `claude`. Empty strings are treated as unset. Reflects the `resolveProviderName` extraction in [qwibitai/nanoclaw#1958](https://github.com/qwibitai/nanoclaw/pull/1958).
  - **`integrations/overview`**: Expanded the `agent_provider` note to document the per-session override path and added `/add-signal` to the available channels list ([qwibitai/nanoclaw#1954](https://github.com/qwibitai/nanoclaw/pull/1954)).
  - **`api/group-management`**: Added `pending_approvals` and `pending_channel_approvals` to the central-DB table list and noted that channel/sender approval rows now persist their own `title` + `options_json` (migration 013).
</Update>
