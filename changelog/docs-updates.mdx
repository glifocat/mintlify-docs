...
<Update label="v2.0.1 → v2.0.11 release sync" description="2026-04-24" tags={["Updated"]}>
  Synced docs to the v2.0.1 → v2.0.11 patch series upstream.

  - **`changelog/index.mdx`**: added per-version entries for v2.0.5 – v2.0.11 reconstructed from commits (upstream `CHANGELOG.md` only tracks 2.0.0)
  - **`introduction.mdx` + `integrations/skills-system.mdx`**: token count bumped from 127k to 130k to match `repo-tokens/badge.svg`
  - **`integrations/overview.mdx`**: added Signal (`/add-signal`) to the channels branch table, and documented the provider resolution precedence (`sessions.agent_provider` → `agent_groups.agent_provider` → `container.json` → `claude`) after the v2.0.9 container-runner fix
</Update>
