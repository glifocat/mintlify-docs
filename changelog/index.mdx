...
<Update label="v1.2.35" description="2026-03-26" tags={["Breaking", "Skill"]}>
  ...
  - Added `/init-onecli` skill — installs OneCLI, configures the Agent Vault gateway, and migrates `.env` credentials into the vault
  - Run `/init-onecli` after updating, or use `/use-native-credential-proxy` to opt back into the legacy `.env`-based proxy
</Update>
...
<Update label="v1.2.22" description="2026-03-24" tags={["Maintenance"]}>
  ...
  - Updated container base image from `node:22-slim` to `node:24-slim`
