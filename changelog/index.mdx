...
<Update label="v1.2.35" description="2026-03-26" tags={["Breaking"]}>
  **OneCLI Agent Vault** replaces the built-in credential proxy. Existing `.env` credentials must be migrated to the vault.

  - Run `/init-onecli` to install OneCLI and migrate credentials
  - The built-in credential proxy is no longer active by default — use `/use-native-credential-proxy` to restore it
</Update>
