...
<Update label="v2.3.0" description="2026-08-24" tags={["Breaking", "Feature", "Fix", "Skill"]}>
  ...
  - **Adapters gain lifecycle hooks and richer declarations** — membership-change, session-created, post-delivery, and wizard hooks, an extend tool, a per-adapter `sessionMode` declaration, and a `decline_notify` unknown-sender policy that declines while telling the sender it did (#3292, #3293, #3294, #3295, #3296, #3297, #3304, #3361)
  - **Per-group runtime tier** — `runtimeTier` (`container` or `vm`) is selected from group config and validated against the driver's isolation capabilities at spawn (#3442)
  ...
  - **Telegram setup improvements** — the pairing card correctly says the code is 6 digits, the approved group connection flow is documented in the skill, and the wizard offers "add another Telegram bot" when one is already configured (#3431, #3352, #3480)
