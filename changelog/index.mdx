...
<Update label="v1.2.43" description="2026-03-30" tags={["Fix"]}>
  - Simplified stale session recovery: instead of immediately retrying with a fresh session, the broken session ID is cleared and the existing exponential backoff in GroupQueue handles the retry. This avoids double container spawns on transient errors.
  - Expanded stale session detection to catch `no conversation found` errors in addition to `ENOENT` and `session not found`
...
<Update label="v1.2.36" description="2026-03-27" tags={["Maintenance", "Fix"]}>
  - Replaced `pino` logger with built-in logger module — removes 2 runtime dependencies
...
<Update label="v1.2.35" description="2026-03-26" tags={["Breaking"]}>
  - **[BREAKING]** OneCLI Agent Vault replaces the built-in credential proxy. Run `/init-onecli` to install OneCLI and migrate credentials. The legacy credential proxy is available as an opt-in skill via `/use-native-credential-proxy`.
