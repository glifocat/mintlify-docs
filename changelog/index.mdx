...
<Update label="v1.2.35" description="2026-03-26" tags={["Breaking", "Security"]}>
  **Breaking:** OneCLI Agent Vault replaces the built-in credential proxy. Existing `.env` credentials must be migrated to the vault.
  ...
  - The built-in credential proxy can be restored via the `/use-native-credential-proxy` skill
  - See the [configuration reference](/api/configuration#credential-environment-variables) for migration details
</Update>
