...
<Update label="v1.2.35" description="2026-03-26" tags={["Breaking", "Security"]}>
  ## Breaking
  - OneCLI Agent Vault replaces the built-in credential proxy — existing `.env` credentials must be migrated to the vault. Run `/init-onecli` to install OneCLI and migrate credentials.

  ## Maintenance
  - Added branch and fork maintenance guidelines (`docs/BRANCH-FORK-MAINTENANCE.md`)
</Update>
