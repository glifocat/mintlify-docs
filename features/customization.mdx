...
- The `.env` file
- Trigger pattern regex in `config.ts`
...
```typescript
// Environment variable precedence in config.ts
...
Generated by `buildVolumeMounts` in `container-runner.ts`.
...
```typescript
// Main group mounts in container-runner.ts
if (isMain) {
  mounts.push({
...
```typescript
// Non-main group mounts in container-runner.ts
mounts.push({
  hostPath: groupDir,
  containerPath: '/workspace/group',
  readonly: false,
