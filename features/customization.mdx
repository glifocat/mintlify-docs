...
// Container settings
// Defaults to `nanoclaw-agent-v2-<slug>:latest`, where <slug> is a
// deterministic hash of the checkout path so two installs don't collide.
// Override via CONTAINER_IMAGE or CONTAINER_IMAGE_BASE in .env.
export const CONTAINER_IMAGE = getDefaultContainerImage(PROJECT_ROOT);
