...
<Note>
The agent sees at most `MAX_MESSAGES_PER_PROMPT` messages per invocation (default: **10**). The underlying database queries cap at 200 rows, but `MAX_MESSAGES_PER_PROMPT` limits how many are actually included in the agent's prompt. Older messages remain in the database but are not sent to the agent. Configure this via the `MAX_MESSAGES_PER_PROMPT` environment variable.
</Note>
