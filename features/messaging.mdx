...
Individual groups can override the trigger via their `trigger` field. When a group has a custom trigger set, only that trigger activates the agent — the default is ignored for that group:

```typescript
// Per-group trigger resolution
const triggerPattern = getTriggerPattern(group.trigger);
// Falls back to DEFAULT_TRIGGER when group.trigger is undefined
```
...
@Claw summarize this thread (custom per-group trigger)
```
...
To change the default trigger word for all groups, ask Claude Code to modify `ASSISTANT_NAME`:
...
To set a custom trigger for a specific group, update the group's `trigger` field during registration. Groups with custom triggers ignore the default — only their specific trigger word activates the agent.
