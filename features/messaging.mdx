...
Trigger patterns determine when the agent should respond. Each group can define its own trigger, with a fallback to the default `@{ASSISTANT_NAME}`.
...
When processing messages for a non-main group, NanoClaw uses the group's `trigger` field to build the regex. If no per-group trigger is set, it falls back to the default (`@{ASSISTANT_NAME}`).
...
@Claw do something (custom per-group trigger)
```
