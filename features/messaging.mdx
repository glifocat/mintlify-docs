...
- Telegram (core channel, available by default)
...
<Warning>
Telegram ships on `main`. Other channels are added via skills, not configuration files. Use `/add-whatsapp` or similar skills to add new channels. See the [integrations overview](/integrations/overview).
</Warning>
