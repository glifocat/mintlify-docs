...
Tasks use the system timezone by default. The timezone is resolved by cascading through `process.env.TZ`, `.env TZ`, and `Intl.DateTimeFormat`, falling back to `UTC`. Each candidate is validated as a real IANA timezone identifier before being accepted.
...
Tasks can include an optional pre-execution script — a bash script that runs inside the container before the agent is invoked. This is useful for lightweight checks that determine whether the agent should run at all.
