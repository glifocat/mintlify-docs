...
For tasks that check a condition before acting (new PRs, website changes, API status), add a `script` parameter. The script runs first as bash inside the container — if there's nothing to do, the agent never starts.

### How scripts work

1. You provide a bash `script` alongside the `prompt` when scheduling a task
2. When the task fires, the script runs first (30-second timeout)
3. The script prints JSON on the last line of stdout: `{ "wakeAgent": true/false, "data": {...} }`
4. If `wakeAgent: false` — nothing happens, the task waits for the next scheduled run
5. If `wakeAgent: true` — the agent wakes up and receives the script's data along with the original prompt

```json
{
  "type": "schedule_task",
  "prompt": "Review these new PRs and summarize them",
...
<Note>
Scripts are only executed for scheduled tasks (`isScheduledTask: true`). The `script` parameter is ignored for interactive messages.
</Note>

### Script output format

The script must print valid JSON on the **last line** of stdout:

```json
{ "wakeAgent": true, "data": { "items": ["a", "b"] } }
```

- `wakeAgent` (required, boolean) — whether to start the agent
- `data` (optional) — arbitrary data passed to the agent's prompt

If the script fails, produces no output, or returns invalid JSON, the agent is **not** woken (fail-safe behavior).

### When to use scripts

Scripts are ideal for **frequent polling** tasks (every 1–10 minutes) where most runs have nothing to report:
...
For tasks that require the agent's judgment every time (daily briefings, reminders, reports), skip the script and use a regular prompt.

<Warning>
Each agent wake-up uses API credits. For tasks running more than a few times per day, use a script to gate wake-ups and avoid unnecessary API costs.
</Warning>

### Updating a task's script

Use `update_task` to add, change, or remove a script:
...
Set `script` to an empty string to remove the script from a task.
...
<Steps>
  ...
  <Step title="Script phase (if configured)">
    If the task has a `script`, it runs first inside the container. If the script returns `wakeAgent: false`, the container exits without starting the agent.
  </Step>
  
  <Step title="Agent executes">
    The agent runs in an isolated container with the task prompt (enriched with script data if applicable)
  </Step>
...
```json update_task
{
  ...
  "script": "optional new script (empty string to remove)",
  "schedule_type": "interval",
...
<Tip>
For high-frequency PR monitoring (every 5–10 minutes), add a script that checks the GitHub API first. The agent only wakes up when there are new PRs to review, saving API credits.
</Tip>
