...
```typescript
// Scheduler loop in task-scheduler.ts
...
```typescript
// Next run computation in ipc.ts (schedule_task handler)
...
```typescript
// Task close delay in task-scheduler.ts
...
```typescript
// Task run logging in task-scheduler.ts
logTaskRun({
  task_id: task.id,
...
```typescript
// Invalid group folder handling in task-scheduler.ts
try {
