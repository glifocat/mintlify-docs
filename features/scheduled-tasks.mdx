...
Tasks use the system timezone by default, with validation to ensure it's a real IANA identifier:

```typescript
// From src/config.ts — cascading fallback with validation
const candidates = [process.env.TZ, envConfig.TZ, Intl.DateTimeFormat().resolvedOptions().timeZone];
// First valid IANA timezone wins; falls back to 'UTC'
```

To override, set the `TZ` environment variable in `.env`:

```bash
TZ=America/New_York
```

Invalid timezone values (e.g., typos like `Amercia/New_York`) are silently skipped in favor of the next candidate.

## Task scripts

Tasks support an optional `script` parameter — a bash script that runs before the agent wakes up. If the script outputs `{ "wakeAgent": false }`, the agent is not invoked. This saves API credits for condition-based tasks.

```
@Andy every hour, check if there are new open PRs on owner/repo. 
Use a script to call the GitHub API. Only wake up if there are new PRs, 
then summarize them for me.
```

The agent will create a task with both a `prompt` and a `script`. On each run:

1. Script runs first (30-second timeout)
2. Script prints JSON: `{ "wakeAgent": true/false, "data": {...} }`
3. If `wakeAgent: true`, the agent receives the data and prompt
4. If `wakeAgent: false` or the script fails, the task completes silently

<Note>
Scripts run inside the container with full Bash access. Always test scripts in the sandbox before scheduling.
</Note>
...
```json schedule_task
{
  ...
  "script": "optional bash script that runs before waking agent",
  "schedule_type": "cron",
  "schedule_value": "0 9 * * *",
  "context_mode": "isolated",
