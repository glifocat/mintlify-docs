...
The `<slug>` is `sha1(projectRoot)[:8]` — a deterministic per-checkout identifier. Two NanoClaw checkouts on the same host get separate service registrations and separate `nanoclaw-agent-v2-<slug>:latest` image tags, so they can't clobber each other on rebuild.

## Container image contents

The agent container (`nanoclaw-agent-v2-<slug>:latest`, per-checkout) is built from `container/Dockerfile`:
