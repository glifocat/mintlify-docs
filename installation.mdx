...
## Per-checkout install identifiers

Each NanoClaw checkout gets a unique 8-character slug derived from `sha1(projectRoot)`, so two installs on the same host coexist without clobbering each other's service registration or container image tag.

| Resource | Name |
|---|---|
| launchd label (macOS) | `com.nanoclaw-v2-<slug>` |
| systemd unit (Linux) | `nanoclaw-v2-<slug>` |
| Container image | `nanoclaw-agent-v2-<slug>:latest` |

The slug is deterministic — the same checkout path always produces the same slug across re-runs. The service management commands below use generic names; substitute your actual label, which setup prints at the end of `bash setup.sh`.
...
<Tabs>
  <Tab title="macOS (launchd)">
    ```bash
    # Resolve this install's label
    LABEL=$(ls ~/Library/LaunchAgents | grep '^com\.nanoclaw-v2-' | sed 's/\.plist$//')
  ...
  <Tab title="Linux (systemd)">
    ```bash
    # Resolve this install's unit (pick the one matching your checkout)
    UNIT=$(systemctl --user list-unit-files | grep -oE 'nanoclaw-v2-[a-f0-9]{8}' | head -1)
...
<Tabs>
  <Tab title="macOS">
    ...
    # Rebuild and restart (substitute your install's label)
    npm run build
  ...
  <Tab title="Linux">
    ```bash
    # Check service status (substitute your install's unit)
    systemctl --user status nanoclaw-v2-<slug>

    # View logs
    journalctl --user -u nanoclaw-v2-<slug> -n 50
