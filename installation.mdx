...
### `pnpm: command not found` during setup

When `corepack enable` fails with an `EACCES` error (common when Node is installed to a system-writable prefix like `/usr/local` that your user doesn't own), `nanoclaw.sh` falls back to `npm install -g pnpm`. The install succeeds, but the resulting binary may not be on your shell's `PATH` — for example, if you've run `npm config set prefix ~/.npm-global` to avoid `sudo`.

`nanoclaw.sh` and `setup.sh` both prepend `$(npm config get prefix)/bin` to `PATH` after the fallback, so the hand-off to `pnpm run setup:auto` works automatically. If you still see `pnpm: command not found` after the install completes, add npm's global bin to your shell config:
