...
- `~/Library/LaunchAgents/com.nanoclaw-v2-<slug>.plist` (macOS) - Service config
- `~/.config/systemd/user/nanoclaw-v2-<slug>.service` (Linux/WSL with systemd) - Service config

<Note>
**Per-checkout slug.** Since v2.0.3, each NanoClaw install gets a deterministic per-checkout slug (first 8 characters of `sha1(projectRoot)`) baked into the launchd label, systemd unit, and Docker image tag. This lets multiple NanoClaw installs coexist on the same host without fighting over shared names. Any v1 `com.nanoclaw` / `nanoclaw.service` unit is left untouched and can run alongside v2.
</Note>

## Service management

NanoClaw runs as a background service. Commands that reference the service name need the per-checkout label — the snippets below auto-discover it.

<Tabs>
  <Tab title="macOS (launchd)">
    ```bash
    # Discover the label for this checkout
    ...
    # Check status (shows all NanoClaw installs)
  ...
  <Tab title="Linux (systemd)">
    ```bash
    # Discover the unit for this checkout
...
# macOS — matches all com.nanoclaw-v2-<slug> installs
launchctl list | grep nanoclaw

# Linux — matches all nanoclaw-v2-<slug> units
systemctl --user list-units 'nanoclaw-v2-*.service'
...
<Tabs>
  <Tab title="macOS">
    ...
    # Rebuild and restart (auto-detects the per-checkout label)
    npm run build
    LABEL=$(launchctl list | awk '/com\.nanoclaw-v2-/ {print $3; exit}')
    launchctl kickstart -k "gui/$(id -u)/$LABEL"
    ```
  ...
  <Tab title="Linux">
    ```bash
    # Check service status (auto-detects the per-checkout unit)
    UNIT=$(systemctl --user list-unit-files 'nanoclaw-v2-*.service' --no-legend | awk 'NR==1 {print $1}')
    systemctl --user status "$UNIT"
