...
interface AgentGroup {
  ...
  agent_provider?: string; // Optional provider override (e.g., 'claude', 'codex', 'opencode')
...
| `pending_approvals` | Generic approval cards (title + options persisted per row) |
| `pending_questions` | Interactive question cards |
| `pending_channel_approvals` | Unknown channel approval flow (persists title + options) |
| `pending_sender_approvals` | Unknown sender approval flow (persists title + options) |
