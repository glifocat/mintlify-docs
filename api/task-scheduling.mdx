...
4. Runs task script if configured (exits early if `wakeAgent: false`)
5. Spawns agent container with task prompt (enriched with script data if applicable)
...
createTask({
  ...
  script: null, // Optional: bash script to run before waking agent
  schedule_type: 'cron',
...
interface ScheduledTask {
  ...
  script?: string | null;              // Optional bash script (runs before agent)
