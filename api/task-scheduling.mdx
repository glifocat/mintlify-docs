...
Tasks can include an optional `script` field containing a bash script that runs inside the container **before** the agent wakes up. This is useful for lightweight checks that don't require a full agent invocation.
