...
<ParamField path="ONECLI_URL" type="string" default="http://localhost:10254">
  URL for the OneCLI gateway that handles credential injection for containers. OneCLI is the sole credential management system as of v1.2.35. Run `/init-onecli` to install and configure.
</ParamField>

<Note>
The built-in credential proxy was removed in v1.2.35. To restore it, apply the `/use-native-credential-proxy` skill (adds `CREDENTIAL_PROXY_PORT` configuration).
</Note>
...
<Note>
API keys and OAuth tokens are no longer stored in `.env`. Secrets are managed via `onecli secrets create` and injected by the OneCLI gateway at request time. The only credential-related variable is `ONECLI_URL`.
</Note>
...
<Accordion title="Credential proxy variables (opt-in skill only)">
  These variables only apply if you've installed the `/use-native-credential-proxy` skill. They are not part of core NanoClaw as of v1.2.35.
