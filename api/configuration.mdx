...
<Tabs>
  ...
  <Tab title="Credential Proxy (legacy / Apple Container)">
    <ParamField path="CREDENTIAL_PROXY_PORT" type="number" default="3001">
      Port for the credential proxy that containers route API calls through. Used by the `/use-native-credential-proxy` skill (Docker) or included automatically with Apple Container.
    </ParamField>
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Credential Proxy (legacy / Apple Container)">
    The credential proxy reads these from `.env` at startup (never exposed to containers). Apple Container users get this behavior by default; Docker users can restore it with the `/use-native-credential-proxy` skill.
