...
<ParamField path="TZ" type="string" default="system timezone">
  Timezone for scheduled tasks and message formatting. Validated as a real IANA identifier. Falls back through `process.env.TZ` → `.env` file → `Intl.DateTimeFormat().resolvedOptions().timeZone` → `UTC`.
</ParamField>
...
Matches messages starting with `@{ASSISTANT_NAME}` (case-insensitive). Individual groups can override this with a per-group trigger via `getTriggerPattern(group.trigger)`, which falls back to the default when no group-specific trigger is set.

## Timezone configuration

Scheduled tasks and message formatting use the configured timezone. The resolution validates each candidate as a real IANA identifier before accepting it:
...
This affects cron expression evaluation for scheduled tasks and local time display in messages. Invalid timezone values (e.g., typos) are silently skipped in favor of the next candidate.
...
<Note>
API keys and OAuth tokens are not stored in `.env`. Secrets are managed via `onecli secrets create` and injected by the OneCLI gateway at request time. The only credential-related variable is `ONECLI_URL`.
</Note>
