...
<ParamField path="OLLAMA_ADMIN_TOOLS" type="boolean" default="false">
  Enable model management tools (`ollama_pull_model`, `ollama_delete_model`, `ollama_show_model`, `ollama_list_running`) in the Ollama MCP server. Only used when the `/add-ollama-tool` skill is installed. When disabled (default), the agent can only list models and generate responses.
</ParamField>
