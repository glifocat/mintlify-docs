...
Each group can override this with its own `trigger` field. At runtime, `getTriggerPattern(group.trigger)` returns the per-group regex, falling back to the default if no custom trigger is set. All triggers are case-insensitive and must appear at the start of the message.
