...
<ParamField path="CREDENTIAL_PROXY_HOST" type="string" default="auto-detected">
  Override the bind address for the credential proxy. Auto-detected based on platform: `127.0.0.1` on macOS/WSL, `docker0` bridge IP on Linux, `0.0.0.0` as fallback.
</ParamField>
...
- Secrets are loaded only by the credential proxy (`credential-proxy.ts`), never exposed to containers
