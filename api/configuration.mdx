...
<ParamField path="CREDENTIAL_PROXY_PORT" type="number" default="3001">
  Port for the credential injection proxy that containers connect to.
</ParamField>
...
<ResponseField name="SENDER_ALLOWLIST_PATH" type="string">
  `~/.config/nanoclaw/sender-allowlist.json` - Sender allowlist for message filtering
</ResponseField>
...
- Secrets are read from `.env` only by `credential-proxy.ts` at startup and `container-runner.ts` for auth mode detection
