...
<ParamField path="ONECLI_API_KEY" type="string">
  API key for authenticating with the OneCLI Agent Vault. Required when connecting to a remote OneCLI gateway; optional for local installs. Tokens follow the `oc_…` format and are validated at setup time.
</ParamField>

<ParamField path="ANTHROPIC_BASE_URL" type="string" default="https://api.anthropic.com">
  Override the Anthropic API endpoint. When set, NanoClaw registers the `claude` provider container config so the agent SDK calls this URL instead of the default. Setup wires this through OneCLI: the matching auth token is stored as a vault secret keyed to the URL hostname, and the container only ever sees `ANTHROPIC_BASE_URL` plus a placeholder `ANTHROPIC_AUTH_TOKEN` — the proxy rewrites the `Authorization: Bearer …` header on the wire.
</ParamField>
...
For custom Anthropic-compatible endpoints, the secret is stored as a generic Bearer credential keyed to the URL hostname:
```bash
onecli secrets create \
  --name Anthropic \
  --type generic \
  --value YOUR_TOKEN \
  --host-pattern your-endpoint.example.com \
