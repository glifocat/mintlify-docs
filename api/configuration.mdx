...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    ...
    # ONECLI_API_KEY=your-key-here  # only if your OneCLI instance requires authentication
    ```

    <Note>
    With the OneCLI Agent Vault, API keys and OAuth tokens are no longer stored in `.env`. Secrets are managed via `onecli secrets create` and injected by the vault at request time. The only credential-related variables are `ONECLI_URL` and optionally `ONECLI_API_KEY`.
    </Note>
  </Tab>
