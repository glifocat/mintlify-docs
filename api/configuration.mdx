...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    <ParamField path="ONECLI_URL" type="string" default="http://localhost:10254">
      URL for the OneCLI Agent Vault gateway that handles credential injection for containers. Run `/init-onecli` to set up.
    </ParamField>
  </Tab>
...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    ...
    <Note>
    With the OneCLI Agent Vault, API keys and OAuth tokens are no longer stored in `.env`. Secrets are managed via `onecli secrets create` and injected by the vault at request time. The only credential-related variable is `ONECLI_URL`. Run `/init-onecli` to migrate existing credentials.
    </Note>
  </Tab>
...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    Credentials are managed externally via OneCLI Agent Vault — no credential environment variables are needed in `.env`.
    ...
    See `onecli secrets list` to verify registered secrets. Run `onecli --help` for the full list of available commands. Use `/init-onecli` to set up the vault and migrate existing `.env` credentials.
  </Tab>
...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    - **Secrets** are never read by NanoClaw — OneCLI Agent Vault manages them externally
    - The vault injects credentials into container API traffic at request time
