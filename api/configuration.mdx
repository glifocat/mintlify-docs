...
<ParamField path="MAX_MESSAGES_PER_PROMPT" type="number" default="10">
  Maximum number of recent messages included in each agent prompt. Messages are pulled from the database since the last agent response. Higher values give agents more context but increase token usage.
</ParamField>

<ParamField path="LOG_LEVEL" type="string" default="info">
  Logging verbosity. Set to `"debug"` to enable verbose container logging (full input/output in container run logs). Other levels: `"trace"`, `"info"`, `"warn"`, `"error"`, `"fatal"`.
</ParamField>
