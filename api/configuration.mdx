...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    ...
    <ParamField path="ONECLI_API_KEY" type="string">
      API key for authenticating with the OneCLI gateway. Read from `.env` or `process.env`. When set, the OneCLI SDK uses this key to authenticate requests to the vault. Optional — most setups rely on the default authentication.
    </ParamField>
  </Tab>
