...
<Tree>
  <Tree.Folder name="skill/telegram branch" defaultOpen>
    ...
    <Tree.File name="src/routing.test.ts" />
...
<Step title="Phase 2: Merge the skill branch">
...
The skill's SKILL.md in the marketplace includes a reference to a setup guide:
...
<Tree>
  <Tree.Folder name=".claude/skills/claw" defaultOpen>
...
This lets the CLI kill the container immediately once output is received, rather than waiting for the Node.js event loop to exit naturally.

Learn more about the claw CLI in the [command-line interface guide](/features/cli).
...
Add `PARALLEL_API_KEY` to `.env` so the OneCLI gateway and MCP env resolution can access it:
