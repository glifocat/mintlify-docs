...
The runtime binary is specified by `CONTAINER_RUNTIME_BIN` in `container-runtime.ts`:
...
Containers are spawned by the `runContainerAgent` function in `container-runner.ts`:
...
From the `buildContainerArgs` function in `container-runner.ts`:
...
Mounts are built per-group in the `buildVolumeMounts` function:
...
From the timeout handling in `runContainerAgent`:
...
<Note>
The hard timeout includes a grace period: it's set to at least `IDLE_TIMEOUT + 30s` so the graceful `_close` sentinel has time to trigger before the hard kill fires.
</Note>
...
On startup, NanoClaw verifies the container runtime is accessible:
...
Orphaned containers from previous runs are cleaned up at startup:
...
From the stdout stream handler in `runContainerAgent`:
