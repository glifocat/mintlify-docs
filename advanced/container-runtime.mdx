...
The runtime binary is hardcoded as `docker` in `src/container-runtime.ts`:
...
The credential proxy bind address is platform-aware:
- **macOS / WSL**: Binds to `127.0.0.1` (Docker Desktop VM routes `host.docker.internal` to loopback)
- **Linux**: Binds to the `docker0` bridge IP so only containers can reach it, falling back to `0.0.0.0`
- **Override**: Set `CREDENTIAL_PROXY_HOST` environment variable

On Linux, `--add-host=host.docker.internal:host-gateway` is automatically added to container args so containers can resolve the host.
...
<Accordion title="Base image and system dependencies">
  ...
  The slim Node.js base keeps the image small while providing the necessary runtime. CJK fonts are included for international character support.
</Accordion>
