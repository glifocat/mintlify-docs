...
<Accordion title="Base image and system dependencies">
  ...
  The slim Node.js 24 base keeps the image small while providing the necessary runtime.
</Accordion>
...
<Steps>
  ...
  <Step title="Pass input via stdin">
    ...
    The input JSON contains the prompt, session ID, group folder, and metadata. Credentials are handled by the OneCLI gateway — never passed via stdin or mounted as files.
  </Step>
...
The `buildContainerArgs` function in `src/container-runner.ts` assembles the Docker CLI arguments. It sets the timezone, applies OneCLI gateway configuration for credential injection, maps the host UID/GID for file permissions, and attaches volume mounts.
...
- OneCLI gateway env vars - Injected by `onecli.applyContainerConfig()`, routes API calls through the gateway for credential injection
