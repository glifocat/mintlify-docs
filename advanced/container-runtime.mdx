...
Mounts are built per-group in `src/container-runner.ts` (`buildVolumeMounts` function):
...
Timeout configuration in `src/container-runner.ts`:
...
On startup, NanoClaw verifies the container runtime is accessible via `src/container-runtime.ts`:
...
Output parsing in `src/container-runner.ts`:
