...
- `agentIdentifier` - Per-group OneCLI agent for credential scoping (non-main groups only)
...
<Tip>
Each group gets its own copy of the agent-runner source in `data/sessions/{group}/agent-runner-src/`. The copy is auto-updated when the upstream source (`container/agent-runner/src/`) has a newer modification time, so upstream improvements propagate automatically. Manual per-group customizations are preserved until the upstream source changes.
</Tip>
