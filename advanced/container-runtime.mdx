...
| Credential handling | OneCLI gateway (default) or credential proxy via `/use-native-credential-proxy` | Built-in credential proxy (`.env`-based) — included in the `apple-container` branch |
| Platform | macOS, Linux, Windows (WSL2) | macOS 15+ only |

<Warning>
The `/use-native-credential-proxy` skill must not be applied when using Apple Container. The `apple-container` branch already includes the credential proxy code — applying the skill on top would create conflicts. During `/setup`, credential collection is handled inline with `.env`-based configuration.
</Warning>
...
<Tabs>
  ...
  <Tab title="Credential Proxy (legacy / Apple Container)">
    ```typescript
    ...
    **Key flags:**
    ...
    <Note>
    Apple Container includes the credential proxy automatically via the `apple-container` branch. Docker users can restore this behavior with the `/use-native-credential-proxy` skill. Do not apply `/use-native-credential-proxy` on an Apple Container installation — it would conflict with the already-included credential proxy code.
    </Note>
  </Tab>
</Tabs>
