...
<Tip>
Each group gets its own copy of the agent-runner source in `data/sessions/{group}/agent-runner-src/`, allowing per-group customization without affecting other groups. The cache is automatically refreshed when the upstream source is newer (based on `index.ts` file modification time), so container image rebuilds propagate updates to all groups.
</Tip>
