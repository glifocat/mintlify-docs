...
<Steps>
  ...
  <Step title="Handle completion">
    ...
    If the container exits with an error indicating a stale or missing session (e.g., missing `.jsonl` file or "no conversation found"), the session is automatically cleared from the database so the next retry starts fresh. See [stale session recovery](/advanced/troubleshooting#stale-session-recovery) for details.
  </Step>
</Steps>
