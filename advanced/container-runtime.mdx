...
| `session_state` | Persistent key/value store. Continuations are keyed per provider (`continuation:<provider>`) so switching providers is lossless — each provider's last thread stays on file |
