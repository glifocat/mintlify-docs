...
The IPC watcher starts at application launch and runs continuously, polling for new files at `IPC_POLL_INTERVAL` (default: 1000ms).
...
Updates an existing task's prompt, schedule type, or schedule value. Recomputes `next_run` if the schedule changes.
...
After refreshing, the host writes an updated groups snapshot to the requesting group's IPC directory.
...
The source group identity is determined by the directory path, not the file contents. This prevents impersonation attacks. The `isMain` flag is looked up from the registered groups database, not derived from the folder name.
...
This is enforced at mount time in `container-runner.ts`:
...
<Tip>
  The IPC poll interval is configurable via `IPC_POLL_INTERVAL` in `config.ts`. The default is 1000ms (1 second). Lowering it reduces latency but increases CPU usage. Raising it saves resources but increases response time.
</Tip>
