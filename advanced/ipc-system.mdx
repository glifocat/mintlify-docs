...
Updates an existing task's prompt, script, schedule type, or schedule value. The next run time is automatically recalculated when the schedule changes.
...
```json
{
  ...
  "script": "#!/bin/bash\necho '{\"wakeAgent\": true, \"data\": {\"status\": \"ok\"}}'",
  "schedule_type": "cron",
  "schedule_value": "0 10 * * *"
}
```

All fields except `taskId` are optional — only include the fields you want to change. Setting `script` to an empty string removes the script from the task.
