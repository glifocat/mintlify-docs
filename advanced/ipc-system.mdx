...
- `script` (optional): Bash script to run before the agent — see [task scripts](/concepts/tasks#task-scripts)
...
All fields except `taskId` are optional — only include the fields you want to change. Set `script` to an empty string to remove an existing script.
