...
<Tip>
  The IPC poll interval is configurable via `IPC_POLL_INTERVAL` in `src/config.ts`. The default is 1000ms. Lowering it reduces latency but increases CPU usage. Raising it saves resources but increases response time.
</Tip>
