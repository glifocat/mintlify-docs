...
**OneCLI gateway** (`@onecli-sh/sdk`):
- Runs on the host as a local gateway (default URL `http://localhost:10254`, configurable via `ONECLI_URL`)
- The gateway intercepts HTTPS traffic and injects API keys or OAuth tokens
- Containers never see real credentials — the gateway handles injection transparently
...
The `onecli.applyContainerConfig()` call injects the necessary environment variables and network configuration into container arguments. If the OneCLI gateway is unreachable, a warning is logged and the container starts without credentials.

<Note>
Containers cannot extract real credentials. The OneCLI gateway intercepts API requests and injects real authentication before forwarding to `api.anthropic.com`.
</Note>
