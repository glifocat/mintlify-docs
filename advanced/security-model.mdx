...
- Container path validation (rejects `..`, absolute paths, and `:` characters to prevent Docker `-v` option injection)
- `nonMainReadOnly` option forces read-only for non-main groups

See `src/mount-security.ts` for the validation implementation.

<Warning>
The mount allowlist is cached in memory after first successful load. Parse or structural errors are permanently cached, but a missing file is rechecked on each access — so you can create `mount-allowlist.json` after service start without restarting. To pick up changes to an existing file, restart the service. This is different from the sender allowlist, which is reloaded on every message cycle.
</Warning>
