...
From `src/container-runner.ts` (`buildVolumeMounts` function):
...
- Proxy reads credentials from `.env` once at startup and determines auth mode
- In API key mode: injects `x-api-key` header on every request
- In OAuth mode: replaces `Authorization: Bearer` header on exchange requests; post-exchange requests pass through
