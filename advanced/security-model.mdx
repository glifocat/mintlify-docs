...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    NanoClaw delegates all credential management to the [OneCLI](https://onecli.sh) Agent Vault. The host process never reads API keys — secrets are registered with OneCLI and injected into container traffic by the vault.
    ...
    **Container environment** (from `src/container-runner.ts`):
    ...
    <Note>
    If the OneCLI Agent Vault is unreachable at container start, the container launches with no credentials. The agent will fail on API calls, and a warning is logged. Re-run after ensuring OneCLI is running (`curl http://127.0.0.1:10254/api/health`). Use `/init-onecli` to set up the vault if not yet configured.
    </Note>
  </Tab>
