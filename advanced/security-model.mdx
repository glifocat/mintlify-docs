...
The main group's project root is mounted read-only. Writable paths the agent needs (store, group folder, IPC, `.claude/`) are mounted separately. This prevents the agent from modifying host application code (`src/`, `dist/`, `package.json`, etc.) which would bypass the sandbox entirely on next restart.

The `store/` directory is mounted separately as read-write at `/workspace/project/store`, giving the main agent direct access to the SQLite database. All other project files remain read-only.
...
<Tabs>
  <Tab title="OneCLI Agent Vault (v1.2.35+)">
    NanoClaw delegates all credential management to the [OneCLI](https://github.com/onecli/onecli) Agent Vault. The host process never reads API keys — secrets are registered with OneCLI and injected into container traffic by the vault.
    ...
    **How it works:**
    ...
    - `ONECLI_API_KEY` (optional) authenticates with the vault when your OneCLI instance requires it

    **Container environment** (from `src/container-runner.ts`):

    ```typescript
    // OneCLI SDK initialized with URL and optional API key
