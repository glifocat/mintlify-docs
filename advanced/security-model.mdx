...
<Warning>
**Breaking change (v1.2.35):** OneCLI Agent Vault is now the sole credential management system. The built-in credential proxy has been removed. Run `/init-onecli` to migrate existing `.env` credentials.
</Warning>

<Tabs>
  <Tab title="OneCLI Gateway (default)">
  ...
  <Tab title="Credential Proxy (opt-in skill)">
    The built-in credential proxy was removed in v1.2.35. To restore it, apply the `/use-native-credential-proxy` skill, which merges the `skill/native-credential-proxy` branch.

    **Credential proxy** (when restored):
    ...
    **Container environment** (when credential proxy is active):
...
│  • Container lifecycle                                            │
│  • Secret injection (OneCLI gateway)                              │
└────────────────────────────────┬─────────────────────────────────┘
                                 │
