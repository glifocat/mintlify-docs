...
The `.env` file is explicitly shadowed by mounting `/dev/null` over it, preventing agents from reading API keys or secrets even though the project root is mounted.
...
- Proxy injects real credentials on every outbound request
- Two auth modes:
  - **API key**: Proxy injects `x-api-key` header on every request
  - **OAuth**: Container CLI exchanges its placeholder token for a temporary API key via the Anthropic OAuth endpoint; proxy injects the real OAuth token on that exchange request
