...
**Issue:** A group's session transcript (`.jsonl`) can go missing after a crash mid-write, manual deletion, or disk-full condition. When the agent tries to resume, it fails with errors like `no conversation found`, `ENOENT` on the `.jsonl` file, or `session not found`.

**Behavior:** NanoClaw automatically detects these stale sessions, clears the broken session ID from both memory and the database, and lets the existing exponential backoff in GroupQueue retry with a fresh session. No manual intervention is needed.

If you see repeated `Stale session detected — clearing for next retry` warnings in the logs and the agent never recovers, check for underlying issues (disk full, permissions) that may prevent new sessions from being created.
