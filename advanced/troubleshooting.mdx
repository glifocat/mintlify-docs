...
# Since v2.0.3 each checkout has a per-install slug; v1 `com.nanoclaw` may also appear if present.
...
<Accordion title="Adjust idle timeout">
  ...
  Then rebuild and restart using the [restart steps below](#rebuild-after-code-changes) (auto-detects the per-checkout label).
</Accordion>
...
<Accordion title="Test mount manually">
  ...
  ```bash
  # Resolve the per-checkout image tag
  IMAGE=$(docker images --format '{{.Repository}}:{{.Tag}}' | grep '^nanoclaw-agent-v2-.*:latest$' | head -1)
  docker run -i --rm \
...
Since v2.0.3 each checkout has a per-install service label — the snippets below auto-discover it.
...
```bash
PLIST=$(ls "$HOME/Library/LaunchAgents"/com.nanoclaw-v2-*.plist 2>/dev/null | head -1)
