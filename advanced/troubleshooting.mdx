...
<Accordion title="Trigger word not matching">
  Each group can have its own trigger. Verify the trigger word in your message matches the group's configured trigger:
  
  ```bash
  sqlite3 store/messages.db "SELECT name, trigger FROM registered_groups;"
  ```
  
  The trigger is case-insensitive and must appear at the start of the message. If a group has a custom trigger (e.g., `@Claw`), the default `@{ASSISTANT_NAME}` trigger will not activate that group.
</Accordion>
