...
# The slug is sha1(projectRoot)[:8] — one row per NanoClaw checkout on this host.
...
<Accordion title="Adjust idle timeout">
  ...
  Then rebuild (substitute your install's label — find it with `launchctl list | grep nanoclaw`):
  ```bash
  npm run build && launchctl kickstart -k gui/$(id -u)/com.nanoclaw-v2-<slug>
  ```
</Accordion>
...
**Symptoms:** `Container exited with code 125: pull access denied for nanoclaw-agent-v2-<slug>` — the container image disappears after a few hours, even though you just built it.

**Cause:** If your container runtime has Kubernetes enabled (Rancher Desktop enables it by default), the kubelet runs image garbage collection when disk usage exceeds 85%. NanoClaw containers are ephemeral (run and exit), so the per-checkout image (`nanoclaw-agent-v2-<slug>:latest`) is never protected by a running container. The kubelet sees it as unused and deletes it — often overnight when no messages are being processed.
...
After updating credentials, restart the service (substitute your install's label):
...
Scan the QR code with your phone, then restart the service (substitute your install's label):
...
<Accordion title="Test mount manually">
  Run a test container to verify mounts (substitute your per-checkout image tag — find it with `docker images | grep nanoclaw-agent-v2`):
...
```bash
# Substitute your install's label (find it with: launchctl list | grep nanoclaw)
launchctl kickstart -k gui/$(id -u)/com.nanoclaw-v2-<slug>
```
...
```bash
launchctl bootout gui/$(id -u)/com.nanoclaw-v2-<slug>
```

### Start the service

```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.nanoclaw-v2-<slug>.plist
```
