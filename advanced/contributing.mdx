---
title: Contributing
description: Guidelines for contributing to NanoClaw, including PR requirements, skill types, and code standards
tag: "UPDATED"
---
...
A [skill](https://code.claude.com/docs/en/skills) is a markdown file that teaches Claude Code how to transform or extend a NanoClaw installation. NanoClaw has four types of skills, each serving a different purpose.
...
<Accordion title="Feature skills contain source changes">
  Feature skills are `skill/*` branches that contain all code changes for the integration — including modifications to `src/`, `container/`, `package.json`, etc. This is by design: the skill branch is the complete diff from `main`.

  Utility skills ship code alongside the SKILL.md but don't modify source files — the code is self-contained in the skill directory. Operational skills are instruction-only.
</Accordion>
...
<Note>
  Slack, Discord, Telegram, and Gmail integrations already exist! Check the [integrations overview](/integrations/overview).
</Note>
...
<Note>
NanoClaw uses ESLint for linting and Prettier for formatting. Run `npm run lint` and `npm run format:check` before submitting. The goal is readability and simplicity.
</Note>
...
NanoClaw maintains skill branches and channel forks that must stay in sync with `main`. For detailed forward-merge procedures, conflict resolution patterns, and fork setup guidelines, see the [branch and fork maintenance guide](https://github.com/qwibitai/nanoclaw/blob/main/docs/BRANCH-FORK-MAINTENANCE.md) in the source repository.
