...
{/* verified-against: CONTRIBUTING.md, docs/skills-model.md, docs/skill-guidelines.md, .claude/skills/{add-telegram,add-codex,add-tavily-tool,manage-mounts}/SKILL.md, .claude/skills/add-telegram/REMOVE.md, container/skills/, src/container-runner.ts, setup/providers/index.ts, setup/channels/run-channel-skill.ts @ 639577c3 */}
...
| **Tool installs** | `/add-tavily-tool` | Wires an MCP server into selected agent groups: pins the `mcp-remote` bridge in the container manifest and registers the server per group with `ncl groups config add-mcp-server`, giving the agent `tavily_search` and `tavily_extract` with no API key. See [Tools](/extend/tools). |
