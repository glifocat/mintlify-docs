...
{/* verified-against: docs/skill-guidelines.md, docs/skill-directives.md, CONTRIBUTING.md, vitest.skills.config.ts, .claude/skills/{add-telegram,manage-mounts,update-skills,add-ollama-tool}/SKILL.md, .claude/skills/add-telegram/REMOVE.md @ 639577c3 */}
{/* verified-against (scoped correction) — structured skill refresh boundaries: .claude/skills/update-skills/SKILL.md, scripts/update-skills.ts @ b76fcb3d */}

A skill is a markdown file that Claude Code executes — there's no engine you must integrate with, so writing one is mostly writing good instructions. The bar upstream holds skills to is [docs/skill-guidelines.md](https://github.com/nanocoai/nanoclaw/blob/main/docs/skill-guidelines.md), and it boils down to three principles: **single responsibility** (one independently useful customization with one cohesive reason to change), **open for extension, closed for modification** (mostly add files; keep reach-ins into existing code to a line or two that calls your own code), and **a test for every functional integration point** (one that goes red if the wiring is deleted or drifts). This page covers the conventions; [the overview](/extend/overview) explains the model.
...
Apply must be **idempotent**: a repeated or interrupted install must complete safely, and a skill that half-applies twice is a bug. `/update-skills` refreshes installed channel and provider payloads through their structured directives: it overwrites skill-owned files and advances dependency and CLI-manifest pins, while skipping credential prompts, `.env` writes, wiring, restarts, and ordinary build/test directives. Validation runs afterward, and any unresolved input or agent fallback blocks completion. Three conventions from the in-tree skills:
