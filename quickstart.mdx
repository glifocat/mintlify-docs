...
<Accordion title="7. Service installation">
  - **macOS**: Creates launchd service at `~/Library/LaunchAgents/com.nanoclaw-v2-<slug>.plist` (the `<slug>` is `sha1(projectRoot)[:8]` so multiple checkouts coexist)
  - **Linux**: Creates systemd service `nanoclaw-v2-<slug>` (user or system level); enables `loginctl linger` for non-root users so the service survives SSH logout
