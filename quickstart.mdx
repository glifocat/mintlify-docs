...
<Accordion title="3. Claude authentication and OneCLI setup">
  - Asks if you have a Claude subscription (Pro/Max) or Anthropic API key
  - Installs and configures the [OneCLI](https://onecli.sh) Agent Vault for secure credential management
  - Credentials are stored in the OneCLI vault (not in `.env`) and injected into containers at request time
  - Run `/init-onecli` later if you need to migrate existing `.env` credentials
</Accordion>
