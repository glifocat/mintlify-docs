...
<Accordion title="3. Claude authentication">
  ...
  - **Docker runtime**: Credentials are registered with OneCLI (`onecli secrets create`) and injected by the gateway at request time — no secrets in `.env`
  - **Apple Container runtime**: Credentials are stored directly in `.env` (`CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`) — the apple-container branch includes a built-in credential proxy that reads from `.env`
  - **Subscription**: Guides you to run `claude setup-token` to get a long-lived token
  - **API key**: Guides you to get a key from console.anthropic.com
