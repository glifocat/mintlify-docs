...
<Accordion title="7. Service installation">
  - **macOS**: Creates a per-checkout launchd service at `~/Library/LaunchAgents/com.nanoclaw-v2-<slug>.plist` (where `<slug>` is a hash of the checkout path, so multiple installs coexist without clobbering each other)
  - **Linux**: Creates a per-checkout systemd service `nanoclaw-v2-<slug>.service` (user or system level); enables `loginctl linger` for non-root users so the service survives SSH logout
