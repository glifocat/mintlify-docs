...
<Accordion title="3. OneCLI and Claude authentication">
  - Installs the OneCLI Agent Vault gateway if not already present
  - Asks if you have a Claude subscription (Pro/Max) or Anthropic API key
  - **Subscription**: Guides you to run `claude setup-token`, then registers the token in the OneCLI vault via `onecli secrets create`
  - **API key**: Registers your `ANTHROPIC_API_KEY` in the OneCLI vault
  - Credentials are managed by the vault and injected at request time — never stored in `.env` or passed to containers
</Accordion>
