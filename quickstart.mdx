...
<Steps>
  ...
  <Step title="Answer the wizard">
    ...
    2. **Your first agent** — start with a **Fresh agent** (the default — shape it by chatting), or stamp one from a [template](/templates/overview): prebuilt agents from the NanoClaw template library, or whatever is already in your local `templates/`. A library pick is copied locally, then stamped. Existing installs skip this prompt.
    ...
    11. **Channel** — connect Telegram (recommended), Discord, WhatsApp, Signal, iMessage, Slack, or Microsoft Teams — or skip and use the terminal. Other channels install after setup with `/add-<name>` in Claude Code.
