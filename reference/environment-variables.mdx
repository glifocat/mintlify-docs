...
| `NANOCLAW_TEMPLATES_DIR` | `templates/` at the project root | `src/config.ts:63` | Local directory [templates](/templates/overview) resolve against. `--template <ref>` is a path relative to it. Resolved once at load — a local path only, never a URL, and never changed at runtime. |
