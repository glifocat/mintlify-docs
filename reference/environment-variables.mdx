...
| `NANOCLAW_TEMPLATE_PATH` | — | `setup/auto.ts:218` | Presets the first-agent [template](/templates/using-templates) as a local ref (e.g. `sales/sdr`), skipping the picker. Same as `--template-path`. Honored even on an existing install, which otherwise never sees the picker. Setup warns and falls back to the picker if the ref isn't found. |
