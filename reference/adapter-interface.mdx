...
| Session created | `registerSessionCreatedHook(hook)` (`src/router.ts`) | Once per brand-new engaged session, with the session, messaging group, platform address, resolved thread id, session mode, and the triggering inbound message — the seam behind per-thread DM thread titles and retiring onboarding affordances. |
| Post-delivery | `registerPostDeliveryHook(hook)` (`src/delivery.ts`) | After each successfully delivered user-facing message (non-system, non-agent, non-task_log), with a `firstDelivery` flag exactly one row per session carries — one-time follow-through on a session's first outbound message. |
