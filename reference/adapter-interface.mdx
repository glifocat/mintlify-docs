...
keywords: ["channel adapter", "ChannelAdapter", "adapter interface", "custom channel", "chat sdk bridge", "registerChannelAdapter", "initChannelAdapters", "supportsThreads", "deliver", "InboundMessage", "setThreadTitle", "membership hook", "post-delivery hook", "extendTool"]
---

{/* verified-against: src/webhook-server.ts @ 639577c3; src/channels/{adapter,channel-registry,chat-sdk-bridge,index}.ts, src/router.ts + src/delivery.ts hook seams, container/agent-runner/src/mcp-tools/server.ts, setup/channels/companions.ts @ 294ef2ae; channels branch src/channels/telegram.ts @ 6ee516ad; raw-text recovery and conversation metadata: src/channels/adapter.ts, src/channels/chat-sdk-bridge.ts @ b76fcb3d */}
...
Selected members of `src/channels/adapter.ts` (see the source for the complete contract):
...
export interface ChannelAdapter {
   ...
   *         (Discord, Slack, Linear, GitHub). Thread/session policy is resolved per wiring; the
...
<Note>
The `setThreadTitle` doc comment above says "the router fires it once"; that comment is stale in source. The router only dispatches the session-created hook (below); the actual `setThreadTitle` caller is a channel-side module registered on that hook (Slack's onboarding module on the `channels` branch). On a trunk-only install nothing calls it.
</Note>
...
| `supportsThreads` | Yes | `true`: the platform supports threads; effective thread handling and session identity depend on the wiring and context. `false`: thread handling cannot be enabled, and the router strips thread ids. This shapes [session identity](/concepts/entity-model). |
...
| `setTyping?` | No | Called by the typing module while an agent is working, and re-fired on an interval to keep platforms' short-lived typing indicators alive. The optional `status` text and `statusKind` (`'auto'` host-generated, `'agent'` agent-authored) let a platform render a working-status line; they are plumbed through the registry and delivery seams but nothing passes them yet. |
| `setThreadTitle?` | No | Titles a conversation thread. Nothing on trunk calls it: a channel-side module fires it from the session-created hook (Slack's onboarding module titles brand-new per-thread DM sessions after the first user message); adapters without a thread-title concept omit it and callers no-op. |
| `setSuggestedPrompts?` | No | Publishes tappable prompt suggestions (title + message pairs) on platforms that support them. |
| `syncConversations?` | No | Returns discovered conversations (`platformId`, `name`, `isGroup`) for bulk import. |
| `resolveConversation?` | No | Resolves conversation type and human-readable metadata for host UI, including group-DM-aware approval rendering. Returns `null` if unavailable. |
| `resolveChannelName?` | No | Legacy name-only fallback for adapters without richer conversation metadata. |
...
export interface ChannelRegistration {
  ...
  /**
   * Wiring-time defaults for this channel, resolvable WITHOUT instantiating
   * the adapter — offline creation paths (setup, init-first-agent, ncl on a
   * host where the factory returned null for missing creds) read them from
   * the registry. Channel modules pass the same const here and to the adapter.
   */
  defaults?: ChannelDefaults;
  containerConfig?: {
...
export interface ChannelContextDefaults {
  ...
  engagePattern?: string;   // required iff engageMode === 'pattern'; may contain the literal {name}
  threads: boolean;         // default thread honoring; must be false when supportsThreads is false
  sessionMode?: 'shared' | 'per-thread'; // omitted means shared
...
- **Raw-text recovery** — pass `extractRawText(raw)` to recover readable text that the SDK adapter leaves only in `message.raw`. Non-empty recovered text is appended to the serialized message body with a blank-line separator before the raw payload is discarded. Without an extractor, the body is unchanged.
...
| Hook | Registered via | Fires |
|---|---|---|
| Membership | `setMembershipHandler(channelType, fn)` (`src/channels/chat-sdk-bridge.ts`) | When a member joins a conversation a bridge instance is in (the SDK's `member_joined_channel`; `left` is reserved but the pinned chat core does not dispatch leave events). One handler per channel type; a second registration overwrites with a warning. |
| Bridge inbound policy | `registerBridgeInboundPolicy(channelType, wrap)` (`src/channels/chat-sdk-bridge.ts`) | At bridge setup: the policy wraps the `ChannelSetup` the bridge hands the SDK, so a module can intercept or decorate every inbound path of that channel type without forking the bridge. |
| Session created | `registerSessionCreatedHook(hook)` (`src/router.ts`) | Once per brand-new engaged session, with the session, messaging group, platform address, resolved thread id, session mode, and the triggering inbound message; it is the seam behind per-thread DM thread titles and retiring onboarding affordances. |
| Post-delivery | `registerPostDeliveryHook(hook)` (`src/delivery.ts`) | After each successfully delivered user-facing message (non-system, non-agent, non-task_log), with a `firstDelivery` flag exactly one row per session carries, for one-time follow-through on a session's first outbound message. |
| Tool extension | `extendTool(name, extension)` (`container/agent-runner/src/mcp-tools/server.ts`) | At agent-runner import time: additively extends a built-in MCP tool's schema, description, and payload passthrough keys instead of editing the base tool's source. Two extensions of one tool merge. |
| Wizard companions | `registerChannelPreStep(channel, step)` and `registerCompanionSkills(channel, skills)` (`setup/channels/companions.ts`) | During the setup wizard: a channel can run a pre-step before its `/add-<channel>` skill and have companion skills materialized alongside it (Slack's auto-provision path registers both). |
