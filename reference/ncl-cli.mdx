...
| create | `ncl groups create --folder <folder> [--name <name>] [--template <ref>] [--timezone <iana-id>] [--id <id>] [--new] [--yes]` | approval | **Without `--template`:** `--folder` required and immutable, `--name` defaults to the folder, idempotent on `--folder` (creating an existing folder returns the existing row), and new ids are prefixed `ag-<uuid>`. **With `--template <ref>`:** stamps from a local [template](/templates/overview) under `templates/`; `--folder` is ignored. The display name is `--name`, else the manifest's `agentName`, else the template ref's leaf folder; the group folder is derived from that name, with a random `-XXXXXXXX` suffix on collision, and the id is a **bare UUID** (no `ag-` prefix). Convergent rather than folder-idempotent: if a group already carries that plugin, the command returns an in-place [update plan](/templates/updating) instead of creating anything — `--yes` applies it, `--id` picks the target when several groups carry it, and `--new` forces a fresh stamp. Reader notices come back as `templateReport` on a create, and as `report` on an update. |
