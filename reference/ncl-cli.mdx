...
| create | `ncl groups create --folder <folder> [--name <name>] [--template <ref>] [--timezone <iana-id>] [--id <id>] [--new] [--yes]` | approval | `--folder` required and immutable (except with `--template`, where the folder derives from the name); `--name` defaults to the folder. Idempotent on `--folder` — creating an existing folder returns the existing row. New ids are prefixed (`ag-<uuid>`). With `--template <ref>`, stamps from a local [template](/templates/overview) under `templates/`; the display name falls back to the template's `agentName`, then the folder. If a group already carries that plugin, the command returns an in-place [update plan](/templates/updating) instead of creating anything — `--yes` applies it, `--id` picks the target when several groups carry it, and `--new` forces a fresh stamp. Skipped or ignored components come back in a `templateReport` array. |
