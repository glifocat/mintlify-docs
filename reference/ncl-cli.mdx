...
`detached_at` is read-only state maintained by channel membership hooks. It records when NanoClaw's bot left a conversation. The row, wiring, destinations, and history remain intact; outbound delivery is refused until the bot rejoins and the hook clears the marker.
...
| `--recurrence` | Cron expression, evaluated in the instance timezone. On `update`, `"null"`/`"none"` clears it (back to one-shot). Recurrences over 4 fires/day are refused unless the task has a `--script` gate or you pass `--dangerously-override-recurrence-limit`. |
