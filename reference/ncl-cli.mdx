...
{/* verified-against: src/cli/dispatch.ts, src/cli/crud.ts, src/cli/client.ts, src/cli/format.ts, src/cli/transport-errors.ts, src/cli/registry.ts, src/cli/frame.ts, src/cli/commands/help.ts, src/cli/resources/{approvals,destinations,dropped-messages,groups,members,messaging-groups,policies,roles,sessions,tasks,user-dms,users,wirings}.ts, src/channels/channel-defaults.ts, src/modules/cross-session-context/{index,history}.ts, bin/ncl @ dce271c6; template behavior @ 20d61af */}

{/* verified-against (scoped correction): src/cli/resources/{tasks,groups}.ts, src/modules/permissions/user-dm.ts, src/channels/chat-sdk-bridge.ts @ b76fcb3d */}
...
- **Positional IDs keep their dashes.** The dispatcher matches the longest registered command that prefixes the words, and everything after it is the target ID — dashes intact. `ncl groups get abc123` and `ncl tasks cancel task-374f-…-442` both resolve; `--id <uuid>` still works everywhere.
...
| `group` (default) | Only `groups`, `sessions`, `destinations`, `members`, `tasks` (plus `help`), and two wiring commands: `wirings get`/`wirings update`, always targeted at the current conversation's wiring — the id is force-filled from the (messaging group, agent group) pair, and updates may only change `engage_mode`/`engage_pattern` (still approval-gated). Group args (`--agent-group-id`, `--group`, and `--id` on groups/destinations) are auto-filled to the caller's group and rejected if they point elsewhere. `list`/`get` results are filtered to the caller's group. Changing `cli_scope` is blocked. |
...
Columns: `channel_type`, `platform_id`, `user_id`, `sender_name`, `reason` (`no_agent_wired` | `no_agent_engaged` | `unknown_sender_strict` | `unknown_sender_request_approval` | `unknown_sender_decline_notify`), `messaging_group_id`, `agent_group_id`, `message_count`, `first_seen`, `last_seen`.
...
| create | `ncl groups create --folder <folder> [--name <name>] [--timezone <iana-id>]` or `ncl groups create --template <ref> [--name <name>] [--timezone <iana-id>] [--id <id>] [--new] [--yes]` | approval | **Without `--template`:** `--folder` required and immutable, `--name` defaults to the folder, idempotent on `--folder` (creating an existing folder returns the existing row), and new ids are prefixed `ag-<uuid>`. **With `--template <ref>`:** stamps from a local [template](/templates/overview) under `templates/`. Combining it with `--folder` is **rejected** — a bare group *is* its folder, a templated group *is* its plugin. The display name is `--name`, else the manifest's `agentName`, else the template ref's leaf folder; the group folder derives from that name, with a random `-XXXXXXXX` suffix on collision. Always creates a new agent, except when updating one this template already made: a group can only be targeted if its workspace holds that plugin's manifest, so a hand-built group or one stamped from another template is rejected. Convergent rather than folder-idempotent: if a group already carries that plugin, the command returns an in-place [update plan](/templates/updating) instead of creating anything — `--yes` applies it, `--id` picks the target when several groups carry it, and `--new` forces a fresh stamp. `--name` and `--timezone` apply only when a group is created; both are ignored on an in-place update. Reader notices come back as `templateReport` on a create, and as `report` on an update. |
...
| config get | `ncl groups config get --id <id>` | open | Shows `agent_group_id`, `provider`, `model`, `effort`, `image_tag`, `assistant_name`, `max_messages_per_prompt`, `skills`, `mcp_servers`, `packages_apt`, `packages_npm`, `additional_mounts`, `cli_scope`, `timezone`, `updated_at`. |
| config update | `ncl groups config update --id <id> [--provider] [--model] [--effort] [--image-tag] [--assistant-name] [--max-messages-per-prompt] [--cli-scope <disabled\|group\|global>] [--timezone <iana-id>]` | approval | At least one flag required. Most changes take effect after `ncl groups restart`; `--timezone` grounds task scheduling immediately (empty string clears it back to the install default). |
| config add-mcp-server | `ncl groups config add-mcp-server --id <id> --name <server> --command <cmd> [--args <json-array>] [--env <json-object>]` or `--url <url> [--headers <json-object>]` | approval | Exactly one of `--command`/`--url`. Server name: 1-64 chars of `[A-Za-z0-9_-]`. `--url` is a remote Streamable HTTP endpoint — HTTPS required (plain HTTP only for localhost / `host.docker.internal`), and URLs with embedded credentials, fragments, or secret-looking query params are rejected. Restart required to take effect. |
...
`add-mcp-server` and `remove-mcp-server` refuse to touch a server owned by a stamped [template](/templates/overview) — update the template and [restamp](/templates/updating) instead. A plugin-owned server removed by hand reappears on the next restamp.
...
| send | `ncl messaging-groups send --channel-type <type> --platform-id <pid> --text <message> [--instance] [--sender-id] [--sender]` | approval |
...
| `--unknown-sender-policy` | enum | Updatable, default declared by the adapter for DM or group context (`strict` for undeclared adapters). `strict` drops silently; `request_approval` sends an approval card to an admin; `decline_notify` politely declines an unknown DM sender and sends an owner or admin a one-line FYI; `public` allows anyone. On group chats, `decline_notify` behaves as `strict` to avoid a public reply. |
...
| history | `ncl sessions history <session-id> [--limit <n>]` | open |
...
| Operation | Invocation | Access |
| --- | --- | --- |
| list | `ncl tasks list [--status <pending\|paused>] [--group <id>] [--session <id>] [--all]` | open |
| get | `ncl tasks get --id <series-id> [--session <id>]` | open |
| create | `ncl tasks create --prompt <text> [--name <name>] [--process-after <when>] [--recurrence <cron>] [--script <bash>] [--group <id>] [--dangerously-override-recurrence-limit]` | open |
| update | `ncl tasks update --id <series-id> [--prompt] [--process-after] [--recurrence] [--script]` | open |
| run | `ncl tasks run --id <series-id>` | open |
| pause / resume | `ncl tasks pause --id <series-id>` / `ncl tasks resume --id <series-id>` | open |
| cancel | `ncl tasks cancel (--id <series-id> \| --all)` | open |
| delete | `ncl tasks delete --id <series-id>` | open |
| append-log | `ncl tasks append-log --msg <text> [--id <series-id>]` | open |

| Flag | Notes |
| --- | --- |
| `--prompt` | Required on create. Must be nonblank when supplied on create or update; omit on update to retain the current prompt. |
| `--process-after` | First/next run time — ISO 8601, offset, or naive-local in the group timezone (falling back to the install timezone). Required for one-shots; with `--recurrence` the first run is derived from the cron grid. |
| `--recurrence` | Cron expression, evaluated in the group timezone (falling back to the install timezone). On `update`, `"null"`/`"none"` clears it (back to one-shot). Recurrences over 4 fires/day are refused unless the task has a `--script` gate or you pass `--dangerously-override-recurrence-limit`. |
...
Connects a messaging group to an agent group — which agent handles which chat. Many-to-many in both directions. Under `group` CLI scope, agents get self-service access to the wiring of their current conversation only: `wirings get` and `wirings update` (limited to `engage_mode`/`engage_pattern`; the id is force-filled, updates stay approval-gated).
...
| Flag | Type | Notes |
| --- | --- | --- |
| `--messaging-group-id` | string | On create: pass this, or the natural key instead (`--channel-type` + `--platform-id` [+ `--instance`]). Not updatable. Create is idempotent on the (messaging group, agent group) pair. |
| `--agent-group-id` | string | On create: pass this or `--agent-group <folder>`. References `agent_groups.id`. Not updatable. |
| `--priority` | number | Updatable, default 0. Fanout order when several agents are wired to one chat — higher first. |
