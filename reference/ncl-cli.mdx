...
| create | `ncl groups create --folder <folder> [--name <name>] [--template <ref>] [--timezone <iana-id>]` | approval | `--folder` required and immutable (except with `--template`, where the folder derives from the name); `--name` defaults to the folder. Idempotent on `--folder` — creating an existing folder returns the existing row. New ids are prefixed (`ag-<uuid>`). |
