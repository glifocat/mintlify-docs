...
| Skill | What it does |
| --- | --- |
| `/slack-a2a-rooms` | Allows sibling NanoClaw bots to exchange mention-driven messages in explicitly allowlisted Slack group DMs, with a consecutive-hop limit. Applied as a companion when the current `/add-slack` payload is installed. |
| `/slack-agent-flow` | Extends `create_agent` in Slack so a new agent gets its own provisioned bot, operator DM, shared room, canvas, and hot-started adapter. This is a separate optional feature skill. |
