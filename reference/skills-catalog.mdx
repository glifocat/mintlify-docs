...
{/* verified-against: main .claude/skills/ and container/skills/ listings + each skill's SKILL.md frontmatter @ 294ef2ae; channels .claude/skills/{add-slack,slack-a2a-rooms,slack-agent-flow,slack-multi-instance}/SKILL.md @ d69c9c5d; current main setup/channels/slack-auto-register.ts @ 001dfa02 */}

{/* verified-against (scoped correction): .claude/skills/ listings, .claude/skills/{add-codex,add-opencode,add-ollama-provider,update-skills}/SKILL.md, setup/channels/{slack-auto-register,run-channel-skill}.ts @ b76fcb3d */}

NanoClaw ships two kinds of skills:

- **Host-side workspace skills** (`.claude/skills/`) — invoked from your coding harness in your NanoClaw checkout. They install channels, providers, and tools, or operate and maintain your install. 52 skills total: 20 channel installs, 3 provider installs, 12 tool installs, 15 operational skills, and 2 Slack feature skills.
...
| `/add-mattermost` | Adds the [Mattermost](/channels/mattermost) bot channel via Chat SDK. Copies a vendored adapter rather than an npm package, offers to reuse a local server when one is running, and can create an evaluation server when none exists |
...
`slack-a2a-rooms` and `slack-agent-flow` have their canonical home on `main` and are included in the count above. `slack-multi-instance` remains on the `channels` branch.
...
| Skill | What it does |
| --- | --- |
| `/add-codex` | Uses Codex (CLI + AppServer) as the full agent provider in place of the Claude Agent SDK — ChatGPT subscription or `OPENAI_API_KEY`, per-group via `ncl groups config update --provider codex` |
| `/add-ollama-provider` | Legacy Ollama integration workflow; its config patches need reconciliation with current database-backed group configuration before use |
| `/add-opencode` | Installs OpenCode as an agent provider, selected per group with `ncl groups config update --provider opencode` — OpenRouter, OpenAI, Google, DeepSeek, and more via OpenCode config |
...
| `/update-skills` | Refreshes installed channel/provider payloads and dependency pins from upstream without repeating credential prompts or channel wiring |
