...
{/* verified-against: main .claude/skills/ and container/skills/ listings + each skill's SKILL.md frontmatter @ 53ea1e8b; channels .claude/skills/{add-slack,slack-a2a-rooms,slack-agent-flow,slack-multi-instance}/SKILL.md @ d69c9c5d */}
...
| Skill | What it does |
| --- | --- |
| `/slack-a2a-rooms` | Allows sibling NanoClaw bots to exchange mention-driven messages in explicitly allowlisted Slack group DMs, with a consecutive-hop limit. Setup applies it as a companion only when run with `--slack-agents`; you can also apply it on top of an existing `/add-slack` install. |
