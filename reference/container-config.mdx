...
| `model` | string | `NULL` (install default model, then provider default) | Agent runner → provider options |
...
Remote HTTP servers can include a `headers` object. Plugin-owned servers cannot be replaced or removed through the CLI or the approval-based self-modification tool; edit the plugin and [restamp](/templates/updating) instead.
