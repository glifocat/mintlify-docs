...
{/* verified-against: src/db/schema.ts, src/db/connection.ts, src/db/session-db.ts, src/session-manager.ts, src/db/migrations/{001-initial,002-chat-sdk-state,module-approvals-pending-approvals,module-agent-to-agent-destinations,module-approvals-title-options,008-dropped-messages,009-drop-pending-credentials,010-engage-modes,011-pending-sender-approvals,012-channel-registration,013-approval-render-metadata,014-container-configs,015-cli-scope,016-messaging-group-instance,017-agent-message-policies,018-approvals-approver-user-id,019-wiring-threads,020-container-config-timezone,021-approval-question,022-messaging-group-detached,023-approvals-instance,index}.ts, src/modules/agent-to-agent/db/agent-message-policies.ts, container/agent-runner/src/db/messages-out.ts, src/modules/scheduling/db.ts @ dce271c6; durable host tables and instance-aware DM resolution: src/db/migrations/024-host-coordination.ts, src/db/coordination.ts, src/host-instance.ts, src/container-runner.ts, src/delivery.ts, src/modules/permissions/user-dm.ts @ b76fcb3d */}

{/* verified-against (scoped correction): src/request-wake.ts, src/db/coordination.ts @ b76fcb3d */}
...
<Note>
The schema is created by migrations (`src/db/migrations/`), not by `src/db/schema.ts` — that file is a reference copy. Migrations are tracked by name in a `schema_version` bookkeeping table, so module-installed migrations can be added without coordinating version numbers. Modules register theirs through `registerMigration()` with owner-qualified names (`module:<module-id>:<migration-id>`); ordering is deterministic — built-in migrations first, then module migrations in module import order.
</Note>
...
| `unknown_sender_policy` | TEXT | `'strict'` | `'strict'` \| `'request_approval'` \| `'decline_notify'` \| `'public'` — what happens when a never-seen sender posts |
| `denied_at` | TEXT | `NULL` | Set when the owner denies channel registration — future messages drop silently (migration 012) |
| `detached_at` | TEXT | `NULL` | Set when NanoClaw's own bot leaves the conversation; delivery is refused until a membership hook clears it on rejoin (migration 022) |
| `created_at` | TEXT | — | ISO timestamp |
...
Cached mapping from (user, channel) to that user's DM messaging group. Lets the host initiate cold DMs (pairing, approval cards) without reprobing the platform API on every send. Populated lazily by `ensureUserDm()` for the default adapter instance. Named instances bypass this instance-less cache and resolve their own DM directly, while reusing the corresponding instance-scoped `messaging_groups` row.
...
Migration 024 adds one row per host process. `instance_id` (TEXT primary key), `install_id`, `started_at`, and `lease_expires_at` are required. `hostname`, `pid`, and `stopped_at` are nullable. A host is live only when it has not stopped and its lease has not expired. This lease participates in session-claim ownership checks.

### session_claims

Migration 024 adds durable per-session ownership and stop intent:

| Column | Type | Default | Meaning |
|---|---|---|---|
| `session_id` | TEXT | — | Primary key |
| `incarnation` | INTEGER | `0` | Container-generation fence; increments on a new start |
| `claimed_by` | TEXT | `NULL` | Host instance owning the claim |
| `claimed_at` | TEXT | `NULL` | Claim timestamp |
| `container_ref` | TEXT | `NULL` | Runtime container reference |
| `stop_intent` | TEXT | `NULL` | `stop` or `respawn_after_stop`, enforced by application code |
| `updated_at` | TEXT | — | Required last-update timestamp |

Spawn, adoption, and completion check claim ownership and incarnation. A stale completion cannot release a newer claim. Stop intent survives a host restart.

### delivery_attempts

Migration 024 stores retry bookkeeping independently of mailbox delivery status. `message_id` (TEXT primary key) and `session_id` are required; `attempts` is an INTEGER defaulting to `0`. Nullable fields are `last_attempt_at`, `next_attempt_at`, and `last_error`. An index covers `session_id`.

The delivery path uses these durable counts across host restarts, with a maximum of three failed attempts. Failure to record an attempt defers the give-up decision and retries on the next poll; it does not treat the message as delivered. `next_attempt_at` exists in the schema but this delivery path currently writes `NULL` rather than using it as a persisted backoff schedule.

### wake_signals

Migration 024 adds a durable wake-signal outbox. Columns are `id` (TEXT primary key), `session_id`, `reason`, `created_at` (all required), and nullable `consumed_at` and `consumed_by`. An index covers `(session_id, consumed_at)`. This schema and its helpers are reserved for durable wake processing: the current `requestWake` path delegates directly to `wakeContainer` and does not write or consume these rows. Runtime event reconciliation and periodic resync operate independently of this table.
...
| `instance` | TEXT | `NULL` | Adapter instance that delivered the card, so later edits resolve the same bot identity; `NULL` means the default instance (`channel_type`) (migration 023) |
...
| `question` | TEXT | `''` | The original card body, persisted so terminal card edits (resolved, timed out — even across a host restart) can re-render it; pre-migration rows fall back to the title (migration 021) |
| `created_at` | TEXT | — | ISO timestamp |
...
| `question` | TEXT | `''` | Original card body for terminal card edits (migration 021) |
| `created_at` | TEXT | — | ISO timestamp |
...
One row per agent group: how its containers spawn (migration 014; `cli_scope` added in 015, `timezone` in 020). See [Container configuration](/reference/container-config) for full field semantics and the [`ncl groups config`](/reference/ncl-cli#groups) commands that edit it.
...
| `timezone` | TEXT | `NULL` | Validated IANA timezone id; `NULL` follows the install-global timezone. Grounds host-side scheduling immediately and the container's `TZ` on the next respawn (migration 020) |
