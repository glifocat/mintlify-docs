...
{/* verified-against: src/templates/{parse,manifest,extension,mcp,skills,tasks,plugin-dir,create-agent,local-dir}.ts, src/container-config.ts, src/modules/self-mod/request.ts, container/agent-runner/src/plugin-mcp.ts @ nanocoai/nanoclaw#3220 f8563abe */}

The precise rules a [template](/templates/overview) is validated against. For a walkthrough of building one, see [Building and submitting a template](/templates/building-and-submitting).
...
| Ref form | Multi-segment relative path — `sales/sdr` → `templates/sales/sdr` |
...
Every entry must declare `type`. `sse` is explicitly rejected as unsupported.
...
| Condition | Result |
| --- | --- |
| Value matches `sk-`, `ghp_`, `github_pat_`, `xox<a>-`, `AKIA`, or `-----BEGIN ` | **Whole template rejected**, with the key named |
| Key matches `TOKEN`, `SECRET`, `PASSWORD`, `API_KEY`, `CREDENTIAL`, `PRIVATE_KEY`, or `AUTH`, and the value is not `"placeholder"` | Warning in `templateReport`; the stamp proceeds |

<Note>
The registry's `check-templates.mjs` **fails** on the second case rather than warning, so a template that stamps cleanly locally can still be rejected by CI. See [Building and submitting a template](/templates/building-and-submitting).
</Note>
...
- there is no `SKILL.md`, or it is a symlink or not a regular file
