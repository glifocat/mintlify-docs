...
{/* verified-against: src/templates/{parse,manifest,extension,mcp,skills,tasks,plugin-dir,create-agent,local-dir}.ts, src/container-config.ts, src/modules/self-mod/request.ts, container/agent-runner/src/plugin-mcp.ts @ nanocoai/nanoclaw#3220 1293ec56 */}
...
<Note>
The registry's `check-templates.mjs` **fails** on the credential-shaped-key case rather than warning, so a template that stamps cleanly locally can still be rejected by CI. It also does not strip auth-scheme prefixes, so a `Bearer sk-…` value trips the engine but not the registry script — though only when the key itself isn't credential-shaped; an `Authorization` header still fails CI on the key rule. See [Building and submitting a template](/templates/building-and-submitting).
</Note>
