...
Not an MCP tool, but documented alongside them in the agent's instructions: the agent also has the `ncl` admin CLI at `/usr/local/bin/ncl` for querying and modifying NanoClaw's central configuration — its own group config, destinations, members, sessions, and the wiring for the current chat (`wirings get`/`update`, limited to `engage_mode` and `engage_pattern`, approval-gated). Read commands are open; write commands return `approval-pending` and execute only after an admin approves. What it can reach is controlled by the group's `cli_scope` — see the [ncl CLI reference](/reference/ncl-cli#access-control).
